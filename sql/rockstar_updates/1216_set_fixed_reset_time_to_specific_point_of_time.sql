...
REPLACE INTO `rockstar`.`server_settings` (`setting_id`, `setting_value`) SELECT CONCAT('next_instance_reset_', `map`.`entry`) AS `setting_id`, (@@TIMESTAMP - 345600 - (@@TIMESTAMP % 604800) - (@TIMEZONE * 3600) + (@DAY_OF_WEEK * 86400) + ((@HOUR_OF_DAY - @TIMEZONE) * 3600)) AS `setting_value`
        FROM `rockstar`.`worldmap_info` AS `map` WHERE `map`.`type` IN (1,4);
