...
bool Player::Create(WorldPacket& data )
{
	...
	SetUInt32Value( PLAYER_FIELD_KNOWN_PVETITLES, 0 );		// 0=disabled
	SetUInt32Value( PLAYER_FIELD_KNOWN_PVPTITLES, 0 );		// 0=disabled
...
#ifdef COLLISION
	if(m_MountSpellId != 0 && RS_getAccess() < RS_GAMEMASTER)
	{
...
void Player::_EventAttack( bool offhand )
{
	...
	if( GetShapeShift() == FORM_FLIGHT || GetShapeShift() == FORM_SWIFT )
	{
	...
	*/
	...
	if(levelup)
	{
		...
		if(info->class_ == WARLOCK) 
		{
			...
			{
				m_Summon->SetUInt32Value(UNIT_FIELD_LEVEL, level);
...
void FixTitles(Player * plr)
{
	switch(plr->GetTeam())
	{
	case TEAM_HORDE:
		{
			CharTitlesEntry * ct = dbcCharTitle.LookupEntryForced(13);
			if( plr->HasTitle( ct, TITLE_TYPE_PVP ) )
			{
				plr->RemoveTitle(ct, TITLE_TYPE_PVP, false, true);
				plr->SetUInt32Value(PLAYER_CHOSEN_TITLE, 0);
			}
		}break;
	case TEAM_ALLIANCE:
		{
			CharTitlesEntry * ct = dbcCharTitle.LookupEntryForced(28);
			if( plr->HasTitle( ct, TITLE_TYPE_PVP ) )
			{
				plr->RemoveTitle(ct, TITLE_TYPE_PVP, false, true); 
				plr->SetUInt32Value(PLAYER_CHOSEN_TITLE, 0);
...
void Player::addSpell(uint32 spell_id)
{
	...
	if(sk)
	{
		...
		uint32 min = 1;
		if(_HasSkillLine(sk->skilline))
			min = _GetSkillLineMax(sk->skilline);
	...
	// add related spells
	for ( SpellSpellRelationMap::iterator it = objmgr.SpellSpellRelationTable.lower_bound( spell_id ); it != objmgr.SpellSpellRelationTable.upper_bound( spell_id ); ++it )
	{
		addSpell( (*it).second );
	}
}
...
void Player::SaveToDB(bool bNewCharacter, bool SaveCmd)
{
	bool in_arena = false;
	...
	*/
	...
	<< m_uint32Values[PLAYER_FIELD_KNOWN_PVPTITLES] << ","
	<< m_uint32Values[PLAYER_FIELD_KNOWN_PVETITLES] << ","
	<< m_uint32Values[PLAYER_FIELD_COINAGE] << ","
	<< m_uint32Values[PLAYER_AMMO_ID] << ","
	<< m_uint32Values[PLAYER_CHARACTER_POINTS2] << ",";

	if (m_uint32Values[PLAYER_CHARACTER_POINTS1] > 76 &&  ! GetSession()->HasGMPermissions())
			SetUInt32Value(PLAYER_CHARACTER_POINTS1, 76);
...
void Player::RemovePendingPlayer()
{
	if(m_session)
	{
		...
		sLog.outDebug("SMSG_CHARACTER_LOGIN_FAILED - RemovePendingPlayer\n");
	}
...
//	uint32 PvPRanks[] = { PLAYER_TITLE_NONE, PLAYER_TITLE_PRIVATE, PLAYER_TITLE_CORPORAL, PLAYER_TITLE_SERGEANT_A, PLAYER_TITLE_MASTER_SERGEANT, PLAYER_TITLE_SERGEANT_MAJOR, PLAYER_TITLE_KNIGHT, PLAYER_TITLE_KNIGHT_LIEUTENANT, PLAYER_TITLE_KNIGHT_CAPTAIN, PLAYER_TITLE_KNIGHT_CHAMPION, PLAYER_TITLE_LIEUTENANT_COMMANDER, PLAYER_TITLE_COMMANDER, PLAYER_TITLE_MARSHAL, PLAYER_TITLE_FIELD_MARSHAL, PLAYER_TITLE_GRAND_MARSHAL, PLAYER_TITLE_SCOUT, PLAYER_TITLE_GRUNT, PLAYER_TITLE_SERGEANT_H, PLAYER_TITLE_SENIOR_SERGEANT, PLAYER_TITLE_FIRST_SERGEANT, PLAYER_TITLE_STONE_GUARD, PLAYER_TITLE_BLOOD_GUARD, PLAYER_TITLE_LEGIONNAIRE, PLAYER_TITLE_CENTURION, PLAYER_TITLE_CHAMPION, PLAYER_TITLE_LIEUTENANT_GENERAL, PLAYER_TITLE_GENERAL, PLAYER_TITLE_WARLORD, PLAYER_TITLE_HIGH_WARLORD, };
	...
	m_uint32Values[PLAYER_FIELD_KNOWN_PVPTITLES]		= get_next_field.GetUInt32();
	m_uint32Values[PLAYER_FIELD_KNOWN_PVETITLES]		= get_next_field.GetUInt32();
	...
	// check PLAYER_CHOSEN_TITLE compatibility with PLAYER_FIELD_KNOWN_PVPTITLES
	// note: PLAYER_FIELD_KNOWN_PVPTITLES updated at quest status loaded

	FixTitles(this);

	// This is broken
	uint32 curTitle = GetUInt32Value(PLAYER_CHOSEN_TITLE);
	if(uint32 titleId = FindTitle(curTitle))
	{
		CharTitlesEntry * ct = dbcCharTitle.LookupEntryForced( titleId  );
		if( !ct || !HasTitle(ct, ( TITLE_TYPE_PVP + TITLE_TYPE_PVE ) ) )
			SetUInt32Value(PLAYER_CHOSEN_TITLE, 0);
	}
...
//#undef _language_fix_
	...
	while(true)
	{
		...
		if(!end) break;
		*end = 0;
		SetUInt32Value(PLAYER_FIELD_DAILY_QUESTS_00 + m_finishedDailies.size(), atol(start));
...
void Player::OnPushToWorld()
{
	...
	*data << uint32(0);
	...
	if( GetSession() )
		sSessionLog.writefromsession(GetSession(), "session logged.");
...
/*
	if(flying_aura && m_mapId != 530 && RS_getAccess() < RS_GAMEMASTER)
	{
...
#ifdef USE_SOFT_INSTANCE_EXIT
	if( m_mapMgr && m_mapMgr->GetMapInfo() && m_mapMgr->GetMapInfo()->type != INSTANCE_NULL && ( m_mapMgr->GetPlayerCount() >= m_mapMgr->GetMapInfo()->playerlimit ) )
	{
		SoftInstanceExit();
	}
#endif
...
{
	// Why MSG_SET_DUNGEON_DIFFICULTY ? CMSG == Client Message...
...
void Player::BuildPlayerRepop()
{
	...
	if(getRace()==RACE_NIGHTELF)
	{
		...
		Spell * sp2 = SpellPool.PooledNew();
		sp2->Init(this,inf,true,NULL);
		sp2->prepare(&tgt);
	}
	else
	{
...
void Player::ResurrectPlayer()
{
	...
	if ( m_resurrecter && IsInWorld()
		&& GetDistanceSq(m_resurrectPosition) < ( detectRange * detectRange )
...
//	tmp = baseDodge[pClass] + float( GetUInt32Value( UNIT_FIELD_STAT1 ) / dodgeRatio[69][pClass] );
	...
	SetFloatValue( PLAYER_DODGE_PERCENTAGE, min( tmp, 65.0f ) );
	...
	SetFloatValue( PLAYER_BLOCK_PERCENTAGE, min( tmp, 50.0f ) );
	...
	SetFloatValue( PLAYER_CRIT_PERCENTAGE, min( cr, 90.0f ) );

	float rcr = tmp + CalcRating( PLAYER_RATING_MODIFIER_RANGED_CRIT ) + ranged_bonus;
	SetFloatValue( PLAYER_RANGED_CRIT_PERCENTAGE, min( rcr, 90.0f ) );
...
bool Player::CanSee(Object* obj) // * Invisibility & Stealth Detection - Partha *
{
	...
	if(getDeathState() == CORPSE) // we are dead and we have released our spirit
	{
		if(object_type == TYPEID_PLAYER)
		{
			...
			if( GmLevel != RS_ADMIN && pObj->m_isGmInvisible == INVIS_ADMIN )
				return false;
...
{	
	...
	{ 
		...
		if ( m_Group && loot_method == PARTY_LOOT_MASTER && m_Group->GetLooter() != m_playerInfo )
			// pass on all ffa_loot and the grey / white items
			if ( !iter->ffa_loot && !(itemProto->Quality < m_Group->GetThreshold()) )
				continue;

		if( (itemProto->Flags & ITEM_FLAG_QUEST) && !(itemProto->QuestId) && !(_HasQuestForItem(itemProto->ItemId)) )
			continue;
...
void Player::_Relocate(uint32 mapid, const LocationVector & v, bool sendpending, bool force_new_world, uint32 instance_id)
{
	...
	if(m_mapId != mapid || force_new_world)
	{
		...
		if(status != INSTANCE_OK)
		{
			...
			if(status == INSTANCE_ABORT_HEROIC_MODE_NOT_AVAILABLE)
				data << uint32(iInstanceType);
	...
	if( m_MountSpellId && RS_getAccess() < RS_GAMEMASTER )
		RemoveAura( m_MountSpellId );
}
#endif
...
void Player::ZoneUpdate(uint32 ZoneId)
{
	...
	if(at && at->category == AREAC_SANCTUARY || at->AreaFlags & AREA_SANCTUARY || at->AreaId == 268)
	{
		Unit * pUnit = (GetSelection() == 0) ? 0 : (m_mapMgr ? m_mapMgr->GetUnit(GetSelection()) : 0);
		if(pUnit != NULL && DuelingWith != pUnit && pUnit->IsPlayer() )
		{
			EventAttackStop();
		...
		if(m_currentSpell)
		{
			...
			if(target != NULL && target != this && target != DuelingWith && target->IsPlayer() && !InSameGroupAs( static_cast<Player*>(target) ) )
				m_currentSpell->cancel();
		}
	}
	...
	if(at->AreaId == 268)
		setFaction(35);
	else if(GetFactionId() == 35 && !bGMTagOn)
	{
		PlayerCreateInfo *info = objmgr.GetPlayerCreateInfo(getRace(), getClass());
	...
	CheckZoneAuras(ZoneId);

	if( IsInWorld() )				// should be
		m_mapMgr->GetStateManager().SendWorldStates(this);
}

void Player::CheckZoneAuras(uint32 ZoneId)
{
	uint32 HellfireSuperiority_Zones[] = { 3836, 3483, 3562, 3713, 3714 };
	for(uint32 x=0;x<MAX_AURAS;x++)
	{
		Aura * aur = m_auras[x];
		if(aur != NULL)
		{
			if( aur->m_spellProto != NULL )
			{
				switch(aur->m_spellProto->Id)
				{
					...
					case 32049:		// Hellfire Superiority
					{
						for(uint32 i=0;i<5;i++)
						{
							if( ZoneId != HellfireSuperiority_Zones[i] )
							{
								aur->Remove();
								return; // Aura is removed, so theres no point in searching any further.
							}
						}
					}break;
				}
...
void Player::ApplyLevelInfo(LevelInfo* Info, uint32 Level)
{
	...
	if(curLevel > 10 && curLevel <= 87)
	{
		if(curLevel < Level)
			ModTalentPoints( Level - curLevel );
		else
		{
	...
	if(TalentPoints == 0)
		TalentPoints = GetUInt32Value(PLAYER_CHARACTER_POINTS1);

	uint32 TalentPointCap = 76;
	uint32 new_talentPoints = min( TalentPoints, TalentPointCap );
	SetUInt32Value(PLAYER_CHARACTER_POINTS1, new_talentPoints);
...
/*
	if(flying_aura && MapID != 530 && RS_getAccess() < RS_GAMEMASTER)
	{
		RemoveAura(flying_aura);
		flying_aura = 0;
	}
...
void Player::SafeTeleport(MapMgr * mgr, const LocationVector & vec)
{
	if( mgr == NULL )
		return;
/*
	if( flying_aura && mgr->GetMapId()!=530 && RS_getAccess() < RS_GAMEMASTER ) 
	{
		RemoveAura(flying_aura);
		flying_aura = 0;
	}
...
void Player::ForcePVPFlag(bool apply)
{
	if(apply)
	{
...
void Player::SetShapeShift(uint8 ss)
{
	...
	for( uint32 x = 0; x < MAX_AURAS + MAX_PASSIVE_AURAS; x++ )
	{
		if( m_auras[x] != NULL )
		{
			...
			{
				if( old_ss > 0 && old_ss != FORM_SHADOW )	// 28 = FORM_SHADOW - Didn't find any aura that required this form
...
void Player::_AddSkillLine(uint32 SkillLine, uint32 Curr_sk, uint32 Max_sk)
{
	...
	if( CheckedSkill->type == SKILL_TYPE_LANGUAGE )
	{
		Curr_sk = ( Curr_sk > 300 ? 300 : ( Curr_sk <1 ? 1 : Curr_sk ) );
		Max_sk = ( Max_sk > 300 ? 300 : Max_sk );
	}
	else if( ( CheckedSkill->type == SKILL_TYPE_PROFESSION || CheckedSkill->type == SKILL_TYPE_SECONDARY ) && CheckedSkill->id != 769 )
	{
		Curr_sk = ( Curr_sk > 375 ? 375 : ( Curr_sk <1 ? 1 : Curr_sk ) );
		Max_sk = ( Max_sk > 375 ? 375 : Max_sk );
	}
	Curr_sk = ( Curr_sk < 1 ? 1 : Curr_sk );
	Max_sk = ( Max_sk < 1 ? 1 : Max_sk );
...
void Player::_UpdateMaxSkillCounts()
{
	...
	for(SkillMap::iterator itr = m_skills.begin(); itr != m_skills.end(); ++itr)
	{
		...
		else if( itr->second.Skill->id == 769 )
		{
			if( new_max > 500 )
				new_max = 500;
		}
		else if( itr->second.Skill->type == SKILL_TYPE_ARMOR )
		{
			if( new_max > 1 )
				new_max = 1;
		}
...
uint32 Player::RS_getAccess()
{
	bool AdminAccess = false;
	bool GMAccess = false;
	bool ScripterAccess = false;
	bool ProbeAccess = false;
	bool UVIPAccess = false;
	bool VIPAccess = false;
	
	if( GetSession()->CanUseCommand('z') )
	{
		AdminAccess = true;
	}
	else if( GetSession()->CanUseCommand('a') )
	{
		GMAccess = true;
	}
	else if( GetSession()->CanUseCommand('b') )
	{
		ScripterAccess = true;
	}
	else if( GetSession()->CanUseCommand('c') )
	{
		ProbeAccess = true;
	}
	else if( GetSession()->CanUseCommand('m') )
	{
		UVIPAccess = true;
	}
	else if( GetSession()->CanUseCommand('2') )
	{
		VIPAccess = true;
	}
	if( AdminAccess ) return RS_ADMIN;
	else if( GMAccess )	return RS_GAMEMASTER;
	else if( ScripterAccess ) return RS_SCRIPTER;
	else if( ProbeAccess ) return RS_PROBEGM;
	else if( AdminAccess || GMAccess || ScripterAccess || ProbeAccess || UVIPAccess ) return RS_UBERVIP;
	else if( VIPAccess ) return RS_VIP;
	else return 0;
}
...
{
	CharacterDatabase.Execute("UPDATE characters SET pvprank = %u, selected_title = %u, available_pvp_titles = %u WHERE guid = %u",
		uint32(GetPVPRank()), m_uint32Values[PLAYER_CHOSEN_TITLE], m_uint32Values[PLAYER_FIELD_KNOWN_PVPTITLES], m_uint32Values[OBJECT_FIELD_GUID]);
}
...
uint32 Player::ActiveDailiesCount()
{
	int count = 0;
	for(uint32 i = 0; i < 25; i++)
	{
		if ( m_questlog[i] && m_questlog[i]->GetQuest()->is_repeatable == QUEST_REPEATABLE_DAILY )
			count++;
...
void Player::Social_AddFriend(const char * name, const char * note)
{
	...
	/* team check
	if( info->team != m_playerInfo->team  && m_session->permissioncount == 0 )
	{
...
uint32 Player::GetMaxPersonalRating()
{
	...
	for (i=0; i<NUM_ARENA_TEAM_TYPES; i++)
	{
		...
		{
			ArenaTeamMember *m = m_playerInfo->arenaTeam[i]->GetMemberByGuid(m_playerInfo->guid);
			if (m)
			{
...
void Player::SoftInstanceExit()
{
//#define USE_SOFT_INSTANCE_EXIT

	if(!this || GetMapMgr() == NULL || GetMapMgr()->GetMapInfo() == NULL)
		return;

	if( GetMapMgr()->GetMapInfo()->type == INSTANCE_NULL )
		return;

	bool RemoveFromInstance = false;
	RemoveFromInstance = ExitInstance();
	if( RemoveFromInstance == false )
	{
		LocationVector LocVec(GetBindPositionX(), GetBindPositionY(), GetBindPositionZ(), GetOrientation());
		SafeTeleport(GetBindMapId(), 0, LocVec);
	}
	else
		return;
}
...
void Player::KnockUp(float f)
{
	...
	data << f;
	data << z;
	GetSession()->SendPacket(&data); // Beam me up Scotty!
	//_plr->SendMessageToSet(&data, true); // Beam me up Scotty!
}

void Player::SetWaterWalk(bool on)
{
	if(on)
		CastSpell(this, 10665, true);
	else
		RemoveAurasDueToSpell(10665);
}

uint32 Player::HasWarns()
{
	uint32 WarnAmount = 0;
	std::stringstream SQL;
	std::string WarnedAcct = GetSession()->GetAccountName().c_str();
	SQL << "SELECT COUNT(*) FROM warnsystem WHERE WarnedAcct = '" << WorldDatabase.EscapeString(WarnedAcct.c_str()) << "'";
	QueryResult * cResult = WorldDatabase.Query( SQL.str().c_str() );
	if( cResult )


	{
		do{
			WarnAmount 	= 	cResult->Fetch()[0].GetUInt32();
...
uint32 Player::HasReports()
{
	uint32 ReportAmount = 0;
	std::stringstream SQL;
	SQL << "SELECT COUNT(*) FROM plr_reports WHERE ReportedChar = '" << GetName() << "'";
	QueryResult * repResult = WorldDatabase.Query( SQL.str().c_str() );
	if( repResult )
	{
		do{
			ReportAmount 	= 	repResult->Fetch()[0].GetUInt32();
...
void Player::GiveItem(uint32 entry, uint32 count)
{
	if( !entry )
		return;
...
/*	if( GetItemInterface()->CanReceiveItem(m_itemProto, count) )
	{
		SendCastResult(SPELL_FAILED_TOO_MANY_OF_ITEM);
		return;
	}*/

	uint32	maxcount		= m_itemProto->MaxCount;
	uint32	to_newstack		= 1;
	uint32	curstack		= 0;
	uint32	to_oldstack		= count;
	
	add = GetItemInterface()->FindItemLessMax(entry, count, false);
	if( add )
	{
		curstack = add->GetUInt32Value(ITEM_FIELD_STACK_COUNT);
		if( curstack + count > maxcount )
		{
			to_oldstack = maxcount - curstack;
			to_newstack = count - to_oldstack;
		}
		else
			to_newstack = 0;

		add->SetUInt32Value(ITEM_FIELD_STACK_COUNT, to_oldstack);
		GetSession()->SendItemPushResult(add,true,false,true,false,GetItemInterface()->GetBagSlotByGuid(add->GetGUID()), 0xFFFFFFFF, to_oldstack);
		add->m_isDirty = true;
	}
	if( to_newstack > 0 )
	{
		slotresult = GetItemInterface()->FindFreeInventorySlot(m_itemProto);
		if(!slotresult.Result)
			 return;

		newItem = objmgr.CreateItem(entry, this);
		AddItemResult result = GetItemInterface()->SafeAddItem(newItem, slotresult.ContainerSlot, slotresult.Slot);
		if(!result)
		{
			delete newItem;
			return;
		}
		newItem->SetUInt64Value(ITEM_FIELD_CREATOR, GetGUID());
		newItem->SetUInt32Value(ITEM_FIELD_STACK_COUNT, to_newstack);
		GetSession()->SendItemPushResult(newItem,true,false,true,true,slotresult.ContainerSlot,slotresult.Slot,to_newstack);
		newItem->m_isDirty = true;
	}
}

bool Player::HasTitle(CharTitlesEntry * ct, uint32 titleType)
{
	if( !ct )
		return false;

	bool hasTitle = false;
	uint32 titleMask = ( uint32(1) << ct->bit_index );
	if(titleType & TITLE_TYPE_PVP && !hasTitle)
	{
		hasTitle = HasFlag(PLAYER_FIELD_KNOWN_PVPTITLES, titleMask);
//		hasTitle = ( GetUInt32Value(PLAYER_FIELD_KNOWN_PVPTITLES) & ( uint32(1) << ct->bit_index ) );
	}
	if(titleType & TITLE_TYPE_PVE && !hasTitle)
	{
		hasTitle = HasFlag(PLAYER_FIELD_KNOWN_PVETITLES, titleMask);
//		hasTitle = ( GetUInt32Value(PLAYER_FIELD_KNOWN_PVETITLES) & ( uint32(1) << ct->bit_index ) );
	}
	return hasTitle;
}

uint32 Player::FindTitle(uint32 index)
{
	if( index == 0 )
		return 0;
	...
	{
		ct = dbcCharTitle.LookupRow(i);
		if( ct != NULL )
			if( index == ct->bit_index )
				return ct->ID;
...
void Player::AddTitle(CharTitlesEntry * ct, uint32 titleType, bool SendNotification)
{
	if( !ct )
		return;
	...
	uint32 titles = 0;
	uint32 titleMask = (uint32(1) << ct->bit_index);
	if( titleType & TITLE_TYPE_PVP )
	{
		titles = GetUInt32Value(PLAYER_FIELD_KNOWN_PVPTITLES);
		titles += titleMask;
		SetUInt32Value(PLAYER_FIELD_KNOWN_PVPTITLES, titles);
	}
	if( titleType & TITLE_TYPE_PVE )
	{
		titles = GetUInt32Value(PLAYER_FIELD_KNOWN_PVETITLES);
		titles += titleMask;
		SetUInt32Value(PLAYER_FIELD_KNOWN_PVETITLES, titles);
	}
	SendTitleUpdate(ct, true, SendNotification);
}

void Player::RemoveTitle(CharTitlesEntry * ct, uint32 titleType, bool SendNotification, bool SoftRemove)
{
	if( !ct )
		return;
	...
	if( !SoftRemove )
		SendTitleUpdate(ct, false, SendNotification);
}

void Player::SendTitleUpdate(CharTitlesEntry * ct, bool add, bool SendNotification)
{
	if( !ct ) return;
	WorldPacket data(SMSG_TITLE_EARNED, 10);
	data << uint32(ct->bit_index) << uint32(add);
	GetSession()->SendPacket( &data );
	if( SendNotification )
	{
		char msg[1024];
		string ti = ct->name;
		ti.replace(ti.find("%s"), 2, GetName());
		sprintf(msg, "Player %s has %s the right to carry the title '%s'", GetName(), (add?"earned":"lost"), ti.c_str());
		sWorld.SendWorldText(msg);
	}
}

bool Player::RemoveTransporterStatus()
{
	if(GetTaxiState())
	{
		sEventMgr.RemoveEvents(this, EVENT_PLAYER_TAXI_DISMOUNT);
	...
	if(m_TransporterGUID)
	{
		...
		if(pTrans)
		{
			if(m_lockTransportVariables == true)
			{
				return false;
			}
			pTrans->RemovePlayer(this);
...
ASCENT_INLINE bool Player::_HasQuestForItem(uint32 itemid)
{
	Quest * qst = NULL;
	for( uint32 i = 0; i < 25; ++i )
	{
		if( GetQuestLogInSlot(i) != NULL )
		{
			qst = GetQuestLogInSlot(i)->GetQuest();
			if( qst != NULL )
			{
				for( uint32 j = 0; j < 5; ++j )
					if( qst->required_item[j] == itemid )
						return true;
			}
		}
	}
...
/*	
	if( GetSession() != NULL )
		GetSession()->SystemMessage("You can only complete 25 more daily quests today.");
*/
	//clear the fields
	for( uint32 i = 0; i < 25; ++i )
...
void Player::GoInvisible(bool enable, uint32 invisType, bool addVisual)
{
	char msg[256];
	snprintf(msg, 256, "Invisibility and Invincibility are now %s and set to %u.", enable?"enabled":"disabled", invisType);
	if( !enable )
	{
		RemoveAura( 24222 );
		m_isGmInvisible = 0;
		m_invisible = false;
		bInvincible = false;
		Social_TellFriendsOnline();
	} else {

		AddAuraByID( 24222 );
		Aura * pAura = FindAura( 24222 );
		if( pAura != NULL )
			pAura->SetNegative();

		m_isGmInvisible = invisType;
		m_invisible = true;
		bInvincible = true;
		Social_TellFriendsOffline();
	}
	//snprintf(msg, 256, "%s You may have to leave and re-enter this zone for changes to take effect.", msg);
	sChatHandler.GreenSystemMessage(GetSession(), (const char*)msg);

	UpdateVisibility();
	//SafeTeleport( GetMapId(), GetInstanceID(), GetPositionX(), GetPositionY(), GetPositionZ(), GetOrientation() );
}

void Player::AddAuraByID(uint32 SpellId)
{
	SpellEntry * info = dbcSpell.LookupEntry(SpellId);
	if( !info )
		return;

	Aura * pAura = AuraPool.PooledNew();
	pAura->Init(info, -1, this, this);
	AddAura( pAura );
}

void Player::DemorphEvent()
{
	DeMorph();
}
