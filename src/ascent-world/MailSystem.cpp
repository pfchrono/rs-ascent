...
//	data << uint32(0x10);
	...
	if( !items.empty( ) )
	{
		...
		data << uint8(itemcount);		// item count
...
void WorldSession::HandleSendMail(WorldPacket & recv_data )
{
	...
	uint32 itemcount2 = itemcount;
	logtext << "Sending " << itemcount2 << " item(s): ";
	...
	for( i = 0; i < itemcount; ++i )
	{
		...
		if( pItem->GetEntry() == 7460000 || pItem->GetEntry() == 7460001 || pItem->GetEntry() == 7460002 || pItem->GetEntry() == 500011 || pItem->GetEntry() == 700016)
		{
			SystemMessage("Sorry, some attached items cannot be sent through mail");
			SendMailError( MAIL_ERR_INTERNAL_ERROR );
			return;
		}
...
*/
	...
	if(!strcmp(player->name, _player->GetName()) && _player->RS_getAccess() < RS_PROBEGM)
	{
		SendMailError(MAIL_ERR_CANNOT_SEND_TO_SELF);
		return;
	}

	if( (msg.stationary == 0x3d || msg.stationary == 0x3d) && _player->RS_getAccess() < RS_PROBEGM)
	{
		SendMailError(MAIL_ERR_INTERNAL_ERROR);
		return;
	}
	...
	// Set up the cost
	int32 cost = 0;
	if( !sMailSystem.MailOption( MAIL_FLAG_DISABLE_POSTAGE_COSTS ) && !( _player->RS_getAccess() >= RS_PROBEGM && sMailSystem.MailOption( MAIL_FLAG_NO_COST_FOR_GM ) ) )
	{
		cost = 30;
	}
	...
	if( !items.empty( ) )
	{
		for( itr = items.begin(); itr != items.end(); ++itr )
		{
			...
			if( !pItem ) continue; // should never be hit.

			uint64 ownerguid = pItem->GetUInt32Value(ITEM_FIELD_OWNER);
			if( ownerguid == NULL || ownerguid != _player->GetGUID() )
			{
				pItem->RemoveFromWorld();
				pItem->DeleteFromDB();
				sCheatLog.writefromsession(_player->GetSession(), "tried to mail an item that had owner==NULL, probable duper!");
				SendMailError(MAIL_ERR_INTERNAL_ERROR);
...
/*
			if( _player->GetItemInterface()->GetItemByGUID(pItem->GetGUID() )
			{
				_player->GetItemInterface()->SafeFullRemoveItemByGuid(pItem->GetGUID());
				pItem->DeleteFromDB();
			}
*/ // Needs further testing. Might solve the mail dupe bug.
			pItem->RemoveFromWorld();
			pItem->SetOwner( NULL );
			...
			if( GetPermissionCount() > 0 )
			{
				...
				logtext << " | ItemId: " << pItem->GetEntry() << " x " << pItem->GetUInt32Value(ITEM_FIELD_STACK_COUNT) << ";";
			}
	...
	logtext << " | and " << msg.money << " copper to player " << player->name;
	//sGMLog.writefromsession(this, logtext.str().c_str());
	sSentMailLog.writefromsession(this, logtext.str().c_str());
...
void WorldSession::HandleTakeMoney(WorldPacket & recv_data )
{
	...
	uint32	curgold		=	_player->GetUInt32Value(PLAYER_FIELD_COINAGE);
	uint32	MailGold	=	message->money;

	if( curgold + MailGold > GOLDLIMIT )
	{
		MailGold = GOLDLIMIT - curgold;
		sChatHandler.SystemMessage(_player->GetSession(), "Can't go over goldlimit!");
		_player->GetItemInterface()->BuildInventoryChangeError(NULL, NULL, INV_ERR_TOO_MUCH_GOLD);
		return;
	}

	_player->ModUnsigned32Value(PLAYER_FIELD_COINAGE, MailGold);

	uint32 newMailGold = (message->money - MailGold);
	if(newMailGold < 0)
		return;

	// message no longer has any money
	message->money = newMailGold;

	// update in sql!
	CharacterDatabase.WaitExecute("UPDATE mailbox SET money = %u WHERE message_id = %u", message->money, message->message_id);
...
void WorldSession::HandleReturnToSender(WorldPacket & recv_data )
{
	...
	// Egari: Let's make this instant instead.
	//message.items.empty() ? (uint32)UNIXTIME : (uint32)UNIXTIME + 3600;
	message.delivery_time = (uint32)UNIXTIME; 
