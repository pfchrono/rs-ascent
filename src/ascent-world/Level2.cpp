...
bool ChatHandler::HandleInvisibleCommand(const char *args, WorldSession *m_session)
{
	...
	{
		pChar->RemoveAuraVisual(24222, 1);
		pChar->m_isGmInvisible = 0;
		pChar->m_invisible = false;
		pChar->bInvincible = false;
		pChar->Social_TellFriendsOnline();
		snprintf(msg, 256, "%s OFF.", msg);
	} else {
		pChar->AddAuraVisual(24222, 1, false);
		pChar->m_isGmInvisible = 1;
...
bool ChatHandler::HandleInvisibleCommand(const char *args, WorldSession *m_session)
{
	...
	uint32 curInvisType = pChar->m_isGmInvisible;
	uint32 InvisType = 0;
	if( *args )
	{
		char* pInvisType = strtok((char*)args, " ");
		if( pInvisType  )
		{
			InvisType = atoi(pInvisType);
			if( curInvisType == InvisType )
				return true;

			if( curInvisType > 0 && InvisType > 0 && curInvisType != InvisType )
			{
				pChar->GoInvisible( true, InvisType, false );
			}
			if( curInvisType == 0 && InvisType > 0 )
				pChar->GoInvisible( true, InvisType );
		}
	}
	else
	{
		if( curInvisType > 0 )
			pChar->GoInvisible( false );
		else
			pChar->GoInvisible( true, 1 );
	}
...
bool ChatHandler::CreateGuildCommand(const char* args, WorldSession *m_session)
{
	...
	Guild * g = objmgr.GetGuildByGuildName( args );
	if( g != 0 )
	{
		SystemMessage(m_session, "A guild with that name already exists." );
		return true;
	}
...
bool ChatHandler::HandleDeleteCommand(const char* args, WorldSession *m_session)
{
	...
	Creature * unit = getSelectedCreature(m_session);
	if(!unit)
	{
	...
	{
		SystemMessage(m_session, "No valid spawn entry found.");
		return true;
	}
	if( unit->IsPet() )
	{
	...
	sGMLog.writefromsession(m_session, "used npc delete, sqlid %u, creature %s, pos %f %f %f", unit->GetSQL_id(), unit->GetCreatureInfo() ? unit->GetCreatureInfo()->Name : "-ERROR-", unit->GetPositionX(), unit->GetPositionY(), unit->GetPositionZ());
...
bool ChatHandler::HandleItemCommand(const char* args, WorldSession *m_session)
{
	...
	Creature * pCreature = getSelectedCreature(m_session);
	if(!pCreature)
	{
		SystemMessage(m_session, "You should select a creature.");
		return true;
	...
	if (amount == -1)
	{
		...
		SystemMessage(m_session, "Syntax: !npc vendoradditem <entry> <amount> - It is auto-saved!!");
		return true;
	}
...
{
	Unit * target = getSelectedUnit(m_session);
	...
	if(target->IsPlayer())
	{
		...
		_player->DealDamage(plr, plr->GetUInt32Value(UNIT_FIELD_HEALTH),0,0,0);
		//plr->SetUInt32Value(UNIT_FIELD_HEALTH, 0);
		plr->KillPlayer();
		BlueSystemMessageToPlr(plr, "%s killed you with a GM command.", _player->GetName());
	}
	else
	{
		// Cast insta-kill.
...
bool ChatHandler::HandleMonsterSayCommand(const char* args, WorldSession *m_session)
{
	...
	{
		Pet * pet = static_cast<Pet*>(crt);
		if(!pet)
			return false;
		
		WorldPacket * data = this->FillMessageData(CHAT_MSG_SAY, LANG_UNIVERSAL, args, pet->GetGUID(), 0);
		pet->SendMessageToSet(data, true);
		delete data;
	}
	else
	{
...
bool ChatHandler::HandleGOState(const char *args, WorldSession *m_session)
{
	...
	char* pState = strtok((char*)args, " ");
	if (!pState)
		return false;

	uint32 state = atoi(pState);
...
bool ChatHandler::HandleGOInfo(const char *args, WorldSession *m_session)
{
	...
	ColorSystemMessage(m_session, MSG_COLOR_SUBWHITE, "------ Object Info: ------");
	SystemMessage(m_session, "%s Entry: |r %s %u |r", MSG_COLOR_GREEN, MSG_COLOR_LIGHTBLUE, GObj->GetEntry());
	SystemMessage(m_session, "%s Model: |r %s %u |r", MSG_COLOR_GREEN, MSG_COLOR_LIGHTBLUE, GObj->GetUInt32Value(GAMEOBJECT_DISPLAYID));
	SystemMessage(m_session, "%s State: |r %s %u |r", MSG_COLOR_GREEN, MSG_COLOR_LIGHTBLUE, GObj->GetUInt32Value(GAMEOBJECT_STATE));
	SystemMessage(m_session, "%s Flags: |r %s %u |r", MSG_COLOR_GREEN, MSG_COLOR_LIGHTBLUE, GObj->GetUInt32Value(GAMEOBJECT_FLAGS));
	SystemMessage(m_session, "%s DynFlags: |r %s %u |r", MSG_COLOR_GREEN, MSG_COLOR_LIGHTBLUE, GObj->GetUInt32Value(GAMEOBJECT_DYN_FLAGS));
	SystemMessage(m_session, "%s Faction: |r %s %u |r", MSG_COLOR_GREEN, MSG_COLOR_LIGHTBLUE, GObj->GetUInt32Value(GAMEOBJECT_FACTION));
	std::stringstream sstext;
	...
	SystemMessage(m_session, "%s Type: |r %s %u -- %s |r", MSG_COLOR_GREEN, MSG_COLOR_LIGHTBLUE, GObj->GetUInt32Value(GAMEOBJECT_TYPE_ID), sstext.str().c_str());
	SystemMessage(m_session, "%s Distance:|r %s %f |r", MSG_COLOR_GREEN, MSG_COLOR_LIGHTBLUE, GObj->CalcDistance((Object*)m_session->GetPlayer()));
	...
	SystemMessage(m_session, "%s Name:|r %s %s |r", MSG_COLOR_GREEN, MSG_COLOR_LIGHTBLUE, GOInfo->Name);
	SystemMessage(m_session, "%s Size:|r %s %u |r", MSG_COLOR_GREEN, MSG_COLOR_LIGHTBLUE, GObj->GetFloatValue(OBJECT_FIELD_SCALE_X));
	SystemMessage(m_session, "%s --------------------------", MSG_COLOR_SUBWHITE);
	return true;
}
