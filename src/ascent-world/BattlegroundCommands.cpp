...
bool ChatHandler::HandleInitializeAllQueuedBGsCommand(const char *args, WorldSession *m_session)
{
...
/*	uint32 first,last,val;
	...
	if(m_session->GetPlayer()->m_bg)
	{
		...
		{
			m_session->GetPlayer()->GetMapMgr()->GetStateManager().CreateWorldState(i, val);
			m_session->GetPlayer()->GetMapMgr()->GetStateManager().UpdateWorldState(i, val);
