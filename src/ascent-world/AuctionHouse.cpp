...
void WorldSession::HandleAuctionPlaceBid( WorldPacket & recv_data )
{
	...
	if( auct == 0 || auct->Owner == _player->GetGUID() || !_player )
	{
		SendAuctionPlaceBidResultPacket(0, AUCTION_ERROR_INTERNAL_REGULAR);
		return;
	}

	if(auct->Owner == _player->GetGUID())
	{
		SendAuctionPlaceBidResultPacket(0, AUCTION_ERROR_CANT_BID_OWN_AUCTION);
		return;
	}
	if(auct->HighestBid > price && price != auct->BuyoutPrice)
	{
		//HACK: Don't know the correct error code... 
		SendAuctionPlaceBidResultPacket(0, AUCTION_ERROR_ALREADY_A_HIGHER_BID);
		return;
	}

	if(_player->GetUInt32Value(PLAYER_FIELD_COINAGE) < price)
	{
		SendAuctionPlaceBidResultPacket(0, AUCTION_ERROR_NOT_ENOUGH_MONEY);
		return;
	}
	...
	_player->SaveToDB(false); // Let's not forget to save the player, stupid exploiters.
}
...
void WorldSession::HandleAuctionSellItem( WorldPacket & recv_data )
{
	...
	if( ( bid == buyout || buyout < bid ) && buyout != 0 )
	{
		WorldPacket data(SMSG_AUCTION_COMMAND_RESULT, 8);
		data << uint32(0);
		data << uint32(AUCTION_CREATE);
		data << uint32(AUCTION_ERROR_INCREMENT_TOO_SMALL);
		SendPacket(&data);
		return;
	}
	...
	if( !pItem || pItem->IsSoulbound() || pItem->HasFlag(ITEM_FIELD_FLAGS, ITEM_FLAG_CONJURED ) )
	{
		...
		data << uint32(0);
		data << uint32(AUCTION_CREATE);
		data << uint32(AUCTION_ERROR_ITEM_NOT_FOUND);
		SendPacket(&data);
		return;
	};
	...
	if (_player->GetUInt32Value(PLAYER_FIELD_COINAGE) < item_deposit)	// player cannot afford deposit
	{
		...
		data << uint32(0);
		data << uint32(AUCTION_CREATE);
		data << uint32(AUCTION_ERROR_NOT_ENOUGH_MONEY);
		SendPacket(&data);
		return;
	}
	...
	// Let's not forget to save the player, stupid exploiters.
	...
	*packet >> rarityCheck >> usableCheck;
	...
	for(; itr != auctions.end(); ++itr)
	{
		...
		if(usableCheck)
		{
			...
			if( plr == NULL || plr->GetItemInterface() == NULL )
				continue;

			uint32 reqLevel = proto->RequiredLevel;// - plr->GetItemInterface()->GetSimplicityMod();
			if(reqLevel && reqLevel > plr->getLevel())
				continue;
