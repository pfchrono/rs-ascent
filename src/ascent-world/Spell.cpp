...
//		return SPELL_DID_HIT_SUCCESS;
	...
	/************************************************************************/
	/*	GameMasters always hit												*/
	/************************************************************************/
	if( p_caster && p_caster->RS_getAccess() >= RS_GAMEMASTER )
		return SPELL_DID_HIT_SUCCESS;
		...
		if(u_caster->IsPlayer())
		{
			if(m_spellInfo->NameHash == SPELL_HASH_FLASH_OF_LIGHT || m_spellInfo->NameHash == SPELL_HASH_HOLY_LIGHT)
			{
				...
				if(TalentRank)
				{
					...
					TalentPct += 1.0f;
					amount *= float2int32( float(TalentPct));
				}
			}
		}
...
void Spell::update(uint32 difftime)
{
	...
	if( ( m_caster &&
		GetProto()->InterruptFlags & CAST_INTERRUPT_ON_MOVEMENT ) &&
...
bool Spell::HasPower()
{
	...
	if (p_caster 
		&& ( p_caster->HasAura(44521) || p_caster->HasAura(44535) || p_caster->HasAura(32727) ) 
		)
		return true;
...
/*	Object* target = NULL;
	...
	else
	{
		...
		if(time <= 100)
			...
			sEventMgr.AddEvent(this, &Spell::HandleEffects, guid, i, EVENT_SPELL_HIT, (uint32)time, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
	}*/
}
...
/*
	...
	if (m_spellInfo->speed > 0)
	{
		...
		if (time<=100)
			...
			sEventMgr.AddEvent(this, &Spell::HandleAddAura, guid, EVENT_SPELL_HIT, (uint32)time, 1, 0); 
	}
	else
		HandleAddAura(guid);
*/
...
uint8 Spell::CanCast(bool tolerate)
{
	...
	if(m_caster->IsInWorld())
	{
		...
		if(GetProto()->MechanicsType == MECHANIC_MOUNTED)
			if(m_caster->GetUInt32Value(UNIT_FIELD_DISPLAYID) != m_caster->GetUInt32Value(UNIT_FIELD_NATIVEDISPLAYID))
		...
		if( target )
		{
			...
			if (m_caster != target 
				&& target->IsPlayer() 
				&& m_caster->IsPlayer()
				&& !(Player*)m_caster->HasFlag(PLAYER_FLAGS, PLAYER_FLAG_GM)
				&& (Player*)target->HasFlag(PLAYER_FLAGS, PLAYER_FLAG_GM)
				)
			...
			else if(GetProto()->NameHash == SPELL_HASH_VINDICATION && !target->IsPlayer() )
				return SPELL_FAILED_BAD_TARGETS;
			else if(GetProto()->NameHash == SPELL_HASH_MEND_PET)
			{
			...
			else if(GetProto()->Id == 18788)
			{
				if( !(target->GetEntry() == 416 
					|| target->GetEntry() == 417 
					|| target->GetEntry() == 1860
					|| target->GetEntry() == 1863
					|| target->GetEntry() == 17252 )
					)
					return SPELL_FAILED_BAD_TARGETS;
			}
		}
	...
	if( p_caster != NULL )
	{
		//Andy: blizzard while moving, looks really sexy
		...
		//Better off having it here so it's made sure it's checked.... (Midnight)
		if( ( p_caster->IsPlayer() )&& ( p_caster->GetMapId() == 13 ) && p_caster->RS_getAccess() < RS_PROBEGM )
			return SPELL_FAILED_SPELL_UNAVAILABLE;
	...
	if(m_targets.m_unitTarget)
	{
		...
		if(target)
		{
			...
			switch(GetProto()->Id)
			{
				...
				{
					uint8 result = CanTame(unitTarget, p_caster);
					if( result != NULL )
					{
						if( result == SPELL_FAILED_UNKNOWN )
						{
				...
				case 36314: //The Seer's Presence
				{
					...
					if(!target || target->IsPlayer() || !target->IsUnit())
						return SPELL_FAILED_BAD_TARGETS;
				...
				{
					if(!target || target->IsPlayer() || target->GetEntry() != 21387)
						return SPELL_FAILED_BAD_TARGETS;
				}break;
				case 35772: // Energy Field Modulator
					{
						if ( !target || target->IsPlayer() || target->GetEntry() != 20774 )
							return SPELL_FAILED_BAD_TARGETS; // castable only on Farahlon Lasher
					} break;
				case 44997: // Converting Sentry
				{
					if( target && ( target->IsPlayer() || ( target->IsCreature() && target->GetEntry() != 24972 ) ) ) // Erratic Sentry
						return SPELL_FAILED_BAD_TARGETS;
				...
				{
					if(!target || !target->IsCreature() || target->GetEntry()!=20058) // target needs to be a Bloodmaul Dire Wolf
						return SPELL_FAILED_BAD_TARGETS;
				}break;
				case 28369: // Gas
				{
					if( !target || !target->IsCreature() || target->GetEntry() != 18879 ) // Phase Hunter
						return SPELL_FAILED_BAD_TARGETS;
				} break;
				...
				{
					if( !target || !target->IsCreature() || target->GetEntry() != 20748) // target needs to be a Thunderlord Dire Wolf
						return SPELL_FAILED_BAD_TARGETS;
				}break;
				case 27907: // Antheol's Disciplinary Rod
				{
					if( !target || !target->IsCreature() || (target->GetEntry() != 15941 && target->GetEntry() != 15945) ) // target needs to be the apprentices.
						return SPELL_FAILED_BAD_TARGETS;
				}break;
				case 41291: // Soul Cannon
				{
					if( !target || !target->IsCreature() || target->GetEntry() != 22357 )
						return SPELL_FAILED_BAD_TARGETS;
				}break;
				case 39810: //Sparrowhawk Net
				{
					if( !target || !target->IsCreature() || target->GetEntry() != 22979 )
						return SPELL_FAILED_BAD_TARGETS;
				}break;
			}
...
*/
		if( p_caster && !p_caster->mSpellOverrideMap.empty() && GetProto() )
		{
...
//	if (!sEventMgr.HasEvent(this, EVENT_SPELL_HIT))
//		delete this;
}
