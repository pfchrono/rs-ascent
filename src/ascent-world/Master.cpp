...
SessionLogWriter* Chat_Log;
SessionLogWriter* Chan_Log;
SessionLogWriter* Session_Log;
SessionLogWriter* MailInsert_Log;
SessionLogWriter* SentMail_Log;
...
#define DEF_VALUE_NOT_SET 0xDEADBEEF
		...
		static const char* default_rockstar_config_file = "ascent-rockstar.conf";
		static const char* default_realm_config_file = "ascent-realms.conf";
		static const char* default_ahbot_config_file = "ascent-ahbot.conf";
#else
		static const char* default_config_file = CONFDIR "/ascent-world.conf";
		static const char* default_rockstar_config_file = CONFDIR "/ascent-rockstar.conf";
		static const char* default_realm_config_file = CONFDIR "/ascent-realms.conf";
		static const char* default_ahbot_config_file = CONFDIR "/ascent-ahbot.conf";
#endif

const char r0ckzt4rz[] = "\n"
"|----------------------------------------------|\n"
"|                      ,-.        _.---._      |\n"
"|                      |  `\\.__.-''       `.   |\n"
"|                       \\  _        _  ..   \\  |\n"
...
bool Master::Run(int argc, char ** argv)
{
	...
	char * rockstar_config_file = (char*)default_rockstar_config_file;
	char * ahbot_config_file = (char*)default_ahbot_config_file;
	...
	while ((c = ascent_getopt_long_only(argc, argv, ":f:", longopts, NULL)) != -1)
	{
		...
		case 'r':
			...
			break;
		case 's':
			rockstar_config_file = new char[strlen(ascent_optarg)];
			strcpy(rockstar_config_file, ascent_optarg);
			break;
		case 'a':
			ahbot_config_file = new char[strlen(ascent_optarg)];
			strcpy(ahbot_config_file, ascent_optarg);
			break;
		case 0:
			break;
		default:
	...
	if( do_check_conf )
	{
		...
		if( Config.RS_Config.SetSource( rockstar_config_file, true ) )
			Log.Success( "Config", "Passed without errors.\n");
		else
			Log.Warning( "Config", "Encountered one or more errors.\n");

		Log.Notice( "Config", "Checking config file: %s\n", ahbot_config_file );
		if( Config.AH_Config.SetSource( ahbot_config_file, true ) )
			Log.Success( "Config", "Passed without errors.\n");
		else
			Log.Warning( "Config", "Encountered one or more errors.\n");
	...
	if(Config.RS_Config.SetSource( rockstar_config_file ))
	{
		Log.Success( "Config", ">> %s", rockstar_config_file );
	}
	else
	{
		Log.Error( "Config", ">> %s", rockstar_config_file );
		return false;
	}
	if(Config.AH_Config.SetSource( ahbot_config_file ))
	{
		Log.Success( "Config", ">> %s", ahbot_config_file );
	}
	else
	{
		Log.Error( "Config", ">> %s", ahbot_config_file );
		return false;
	}
	...
	new EventMgr;
	new World;
	/// Have to init these for singleton
	...
	Chat_Log = new SessionLogWriter(FormatOutputString( "logs", "chat", true).c_str(), false );
	Chan_Log = new SessionLogWriter(FormatOutputString( "logs", "chan", true).c_str(), false );
	Session_Log = new SessionLogWriter(FormatOutputString( "logs", "sessions", false).c_str(), false );
	MailInsert_Log = new SessionLogWriter(FormatOutputString( "logs", "mailinsert", false).c_str(), false );
	SentMail_Log = new SessionLogWriter(FormatOutputString( "logs", "sent_mail", false).c_str(), false );
