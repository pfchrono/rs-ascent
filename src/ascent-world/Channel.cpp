...
Channel::Channel(const char * name, uint32 team, uint32 type_id, uint32 id)
{
	...
	// scape stuff
	if( !stricmp(name, "world") || !stricmp(name, "gmhelp") )
	{
		m_minimumLevel = 5;
		m_general = true;
		m_announce = false;
...
void Channel::AttemptJoin(Player * plr, const char * password)
{
	...
	if(voice_enabled)
	{
		...
		data.Initialize(SMSG_AVAILABLE_VOICE_CHANNEL);
...
void Channel::SetOwner(Player * oldpl, Player * plr)
{
	...
	if(oldpl != NULL)
	{
		...
		if(!(itr->second & CHANNEL_FLAG_OWNER) && itr->first->RS_getAccess() < RS_GAMEMASTER)
		{
...
void Channel::Say(Player * plr, const char * message, Player * for_gm_client, bool forced)
{
	...
	if(!forced)
	{
		...
		if( plr->getLevel() < m_minimumLevel && plr->RS_getAccess() < RS_VIP )
		{
...
{
	return GetChannel(name, p->GetTeam());
}
...
void Channel::SendVoiceUpdate()
{
	...
	*/
	...
	WorldPacket data(SMSG_VOICE_SESSION_ROSTER_UPDATE, 300);
