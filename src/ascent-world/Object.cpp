...
#include "Unit.h"
using namespace std;
#pragma warning(disable : 4018 244)
...
static float DamageToRageConversionTable[81]=
{
	0.0f,
	3.33333332596f,
	2.32494760373f,
...
void Object::OutPacketToSet(uint16 Opcode, uint16 Len, const void * Data, bool self)
{
	...
	for(; itr != it_end; ++itr)
	{
		...
		if( gm )
		{
			if( (*itr)->RS_getAccess() >= RS_PROBEGM )
...
void Object::SendMessageToSet(WorldPacket *data, bool bToSelf, bool myteam_only, uint64 ignoreguid)
{
	...
	std::set<Player*>::iterator itr = m_inRangePlayers.begin();
	std::set<Player*>::iterator it_end = m_inRangePlayers.end();
	bool gminvis = (m_objectTypeId == TYPEID_PLAYER ? ( static_cast< Player* >( this )->m_isGmInvisible != 0 ) : false);
	...
	if(myteam_only)
	{
		...
		if(gminvis && data->GetOpcode() != SMSG_MESSAGECHAT)
		{
			...
			{
				ASSERT((*itr)->GetSession());
				if((*itr)->RS_getAccess() >= RS_PROBEGM && (*itr)->GetTeam()==myteam)
...
void Object::ModUnsigned32Value(uint32 index, int32 mod)
{
	...
	if( index == PLAYER_FIELD_COINAGE )
	{
		if( m_uint32Values[index] + mod > ( 210000 * MONEY_ONE_GOLD ) )
		{
			mod = ( ( 210000 * MONEY_ONE_GOLD ) - m_uint32Values[index] );
		}
	}

	m_uint32Values[ index ] += mod;
	if(index < ITEM_FIELD_SPELL_CHARGES && index > ITEM_FIELD_SPELL_CHARGES_04)
	{
...
void Object::_setFaction()
{
	...
	if(GetTypeId() == TYPEID_UNIT || GetTypeId() == TYPEID_PLAYER)
	{
		...
		if( !factT )
			sLog.outDebug("Unit does not have a valid faction, faction %u set for entry %u", GetUInt32Value(UNIT_FIELD_FACTIONTEMPLATE), GetUInt32Value(OBJECT_FIELD_ENTRY) );
	}
	else if(GetTypeId() == TYPEID_GAMEOBJECT)
	{
		factT = dbcFactionTemplate.LookupEntry(GetUInt32Value(GAMEOBJECT_FACTION));
		if( !factT )
			sLog.outDebug("Game Object does not have a valid faction., faction %u set for entry %u", GetUInt32Value(GAMEOBJECT_FACTION), GetUInt32Value(OBJECT_FIELD_ENTRY) );
	}
...
void Object::DealDamage(Unit *pVictim, uint32 damage, uint32 targetEvent, uint32 unitEvent, uint32 spellId, bool no_remove_auras)
{
	...
	pVictim->pLastAttacker = this; // PyRo - 1/10/08
	...
	if( pVictim->IsPlayer() && IsPlayer() )
	{
		if( isHostile( this, pVictim ) && static_cast< Player* >( pVictim )->DuelingWith != static_cast< Player* >( this ) )
		{
			...
			static_cast< Player* >( this )->SetPVPCombat();
		}
	}
...
//========================================================================================== 
	...
	// School damage is not necessarily magic
...
void Object::SendAttackerStateUpdate( Object* Caster, Object* Target, dealdamage *Dmg, uint32 Damage, uint32 Abs, uint32 BlockedDamage, uint32 HitStatus, uint32 VState )
{
	...
	if(Target->IsUnit() && !static_cast<Unit*>(Target)->isAlive())
		return;
...
{
	sLog.outError("ERROR: Attempt %s non-existed value field: %u (count: %u) for object typeid: %u",(set ? "set value to" : "get value from"),index, m_valuesCount, GetTypeId());
