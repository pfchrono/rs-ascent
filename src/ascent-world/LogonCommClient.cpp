...
void LogonCommClientSocket::HandlePacket(WorldPacket & recvData)
{
	static logonpacket_handler Handlers[RMSG_COUNT] = {
		...
		&LogonCommClientSocket::HandleReloadAccounts,		// RCMSG_RELOAD_ACCOUNTS
...
void LogonCommClientSocket::HandleReloadAccounts(WorldPacket & recvData)
{
	WorldPacket data(RCMSG_RELOAD_ACCOUNTS, 50);
	data << uint32(31);
	data << sWorld.logon_pass;
	data << uint32(42);
