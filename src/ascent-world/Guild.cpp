...
void Guild::AddGuildLogEntry(uint8 iEvent, uint8 iParamCount, ...)
{
	...
	if(m_log.size() >= 25)
	{
		// limit it to 25 events.
		// delete the first (oldest) event.
		GuildLogEvent * ev2 = *(m_log.begin());
		CharacterDatabase.Execute("DELETE FROM guild_logs WHERE log_id = %u AND guildid = %u", ev2->iLogId, m_guildId);
		delete ev2;
		m_log.pop_front();
	}
...
GuildRank * Guild::CreateGuildRank(const char * szRankName, uint32 iPermissions, bool bFullGuildBankPermissions)
{
	...
	for(i = 0; i < MAX_GUILD_RANKS; ++i)
	{
		if(m_ranks[i] == NULL)
		{
			...
			CharacterDatabase.Execute("INSERT INTO guild_ranks VALUES(%u, %u, \"%s\", %u, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d)",
			...
			Log.Debug("Guild", "Created rank %u on guild %u (%s) with rights %u", i, m_guildId, szRankName, r->iRights);
...
void Guild::SetMOTD(const char * szNewMotd, WorldSession * pClient)
{
	...
	if(strlen( CharacterDatabase.EscapeString(string(szNewMotd)).c_str() ) > 200)
	{
		pClient->SendNotification("Maximum length is 300 characters.");
		return;
	}
	if(m_motd)
		free(m_motd);
...
void Guild::SetGuildInformation(const char * szGuildInformation, WorldSession * pClient)
{
	...
	if(strlen( CharacterDatabase.EscapeString(string(szGuildInformation)).c_str() ) > 200)
	{
		pClient->SendNotification("Maximum length is 200 characters.");
		return;
	}
	if(m_guildInfo)
		free(m_guildInfo);
...
void Guild::BuyBankTab(WorldSession * pClient)
{
	...
	pTab->szTabName=NULL;
	pTab->szTabIcon=NULL;
	pTab->szTabText=strdup("No Information.\0");

	m_bankTabs.push_back(pTab);
	m_bankTabCount++;

	CharacterDatabase.Execute("INSERT INTO guild_banktabs VALUES(%u, %u, '', '', 'No Information.')", m_guildId, (uint32)pTab->iTabId);
...
{
	if(pRank->iGoldLimitPerDay == -1 || pRank->iId == 0)		// Unlimited
		return 0x7FFFFFFF;
...
void GuildMember::OnMoneyWithdraw(uint32 amt)
{
	...
	{
		// increment counter
		// uWithdrawlsSinceLastReset++; -- Who the f*ck coded this? Seriously...
		uWithdrawlsSinceLastReset = pPlayer->guildMember->uWithdrawlsSinceLastReset + amt;
...
{
	if(pClient->GetPlayer()->GetUInt32Value(PLAYER_FIELD_COINAGE) < uAmount || uAmount == 0)
		return;

	if(m_bankBalance + uAmount > 0x7FFFFFFF)
	{
		pClient->SendNotification("Your Guild's Gold Vault is full!");
		sChatHandler.SystemMessage(pClient, "The maximum capacity of the vault is %u copper.", 0x7FFFFFFF);
		return;
	}
...
// Guild Bank Tab Text
void Guild::SendGuildBankTabInfo(WorldSession * pClient,uint8 TabId) 
{
	...
	if( !tab )
		return;
	WorldPacket data(MSG_QUERY_GUILD_BANK_TEXT, 2+strlen(tab->szTabText));
	data << uint8(TabId);
	data << tab->szTabText;
	pClient->SendPacket(&data);
}

void Guild::SetGuildBankTabInfo(WorldSession * pClient,uint8 TabId,string text)
{
	if( TabId > MAX_GUILD_BANK_TABS )
		return;
	...
	if( text.length() == 0 )
	{
		char* ptmp = tab->szTabText;
		tab->szTabText = strdup("No Information.\0");

		if( ptmp )
			free( ptmp );

		SendGuildBankTabInfo(pClient, TabId);
		CharacterDatabase.Execute("UPDATE guild_banktabs SET tabText = \"No Information.\" WHERE guildId = %u AND tabId = %u",
			GetGuildId(), (uint32)TabId);

		return;
	}

	if( tab->szTabText && strncmp( text.c_str(), tab->szTabText, 100 ) == 0 )
		return;
	...
	SendGuildBankTabInfo(pClient,TabId);
	CharacterDatabase.Execute("UPDATE guild_banktabs SET tabText = \"%s\" WHERE guildId = %u AND tabId = %u",
		CharacterDatabase.EscapeString(text).c_str(),GetGuildId(), (uint32)TabId);
}
