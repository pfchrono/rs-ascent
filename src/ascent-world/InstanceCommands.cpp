...
bool ChatHandler::HandleResetInstanceCommand(const char* args, WorldSession *m_session)
{
	...
	{
		PlayerInfo * pInfo = plr->m_playerInfo;
		if(m_session->CanUseCommand('z') && pInfo != NULL)
		{
			bool foundSomething = false;
			pInfo->savedInstanceIdsLock.Acquire();
			for(int difficulty=0; difficulty<NUM_INSTANCE_MODES; difficulty++)
			{
				PlayerInstanceMap::iterator itr =pInfo->savedInstanceIds[difficulty].find(instance->m_mapId);
				if(itr == pInfo->savedInstanceIds[difficulty].end() || (*itr).second != instance->m_instanceId)
					continue;
			...
			pInfo->savedInstanceIdsLock.Release();
			if(!foundSomething)
...
bool ChatHandler::HandleShowInstancesCommand(const char* args, WorldSession* m_session)
{
	...
	if(pInfo != NULL)
	{
		...
		for(int difficulty=0; difficulty<NUM_INSTANCE_MODES; difficulty++)
		{
			for(PlayerInstanceMap::iterator itr = pInfo->savedInstanceIds[difficulty].begin(); itr != pInfo->savedInstanceIds[difficulty].end(); ++itr)
			{
				count++;
