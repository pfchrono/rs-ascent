...
public:
	...
	//
	void CreateBugReport(const char * args);
	...
	void SendPetNameInvalid(uint32 error, std::string name);
	bool isValidPetName( string NewName );
...
protected:
	...
	void HandleGuildQueryBankTabInfo(WorldPacket & recv_data);
	void HandleGuildSetBankTabInfo(WorldPacket & recv_data);
	...
	void HandleReportPlayerAFKOpcode(WorldPacket &recvPacket);
...
public:
	...
	void CastOnAllPlayers(uint32 spellid);
