...
int8 ItemInterface::CanEquipItemInSlot(int8 DstInvSlot, int8 slot, ItemPrototype* proto, bool ignore_combat /* = false */, bool skip_2h_check /* = false */)
{
	...
	if((slot < INVENTORY_SLOT_BAG_END && DstInvSlot == INVENTORY_SLOT_NOT_SET) || (slot >= BANK_SLOT_BAG_START && slot < BANK_SLOT_BAG_END && DstInvSlot == INVENTORY_SLOT_NOT_SET))
	{
		...
		if( IsEquipped(proto->ItemId) && proto->Flags & ITEM_FLAG_UNIQUE_EQUIP )
			return INV_ERR_ITEM_UNIQUE_EQUIPPABLE;

		if( proto->Unique && EquippedCount(proto->ItemId) >= proto->Unique ) 
			return INV_ERR_ITEM_UNIQUE_EQUIPPABLE;
		...
		if( ( proto->RequiredLevel - GetSimplicityMod() ) > m_pOwner->GetUInt32Value(UNIT_FIELD_LEVEL))
...
enum CanAffordItem
{
	...
	CAN_AFFORD_ITEM_ERROR_UNK0					= 3,
	CAN_AFFORD_ITEM_ERROR_DOESNT_LIKE_YOU		= 4,
	CAN_AFFORD_ITEM_ERROR_TOO_FAR_AWAY			= 5,
	CAN_AFFORD_ITEM_ERROR_UNK1					= 6,
	CAN_AFFORD_ITEM_ERROR_UNK2					= 7,
	CAN_AFFORD_ITEM_ERROR_CANT_CARRY_ANY_MORE	= 8,
	CAN_AFFORD_ITEM_ERROR_UNK3					= 9,
	CAN_AFFORD_ITEM_ERROR_UNK4					= 10,
		...
		if( m_pItems[i] != NULL && m_pItems[i]->GetProto()->BagFamily == 0 && m_pItems[i]->IsContainer() ) //special bags ignored
		{
			for( uint32 j =0; j < m_pItems[i]->GetProto()->ContainerSlots; j++ )
			{
				...
				if( item2 == NULL )
				{
					if(item->GetOwner()->isCasting())
					{
						uint32 CastingSpell = item->GetOwner()->GetCurrentSpell()->GetProto()->Id;
						if(item->GetOwner()->isCasting())
						{
							if(item->GetOwner()->GetCurrentSpell()->castedItemId == item->GetEntry())
								item->GetOwner()->InterruptSpell();
...
uint32 ItemInterface::EquippedCount(uint32 itemid)
{
	uint32 count = 0;
	for( uint32 x = EQUIPMENT_SLOT_START; x < EQUIPMENT_SLOT_END; ++x )
	{
...
// GemID: Gem Item ID
// IgnoreSlot: ignore the item which is equipped in this slot
bool ItemInterface::HasGemEquipped( uint32 GemID , int8 IgnoreSlot /* = -1 */ )
{
	...
	for( uint32 x = EQUIPMENT_SLOT_START; x < EQUIPMENT_SLOT_END; ++x )
	{
		...
		if(m_pItems[x] != NULL)
		{
			socketcount = m_pItems[x]->GetSocketsCount();

			if( socketcount > 0 )
			{
				for(uint32 i = 0; i < socketcount; i++)
				{
					if( m_pItems[x]->GetEnchantment(2+i) && m_pItems[x]->GetEnchantment(2+i)->Enchantment )
					{
						if( m_pItems[x]->GetEnchantment(2+i)->Enchantment->GemEntry == GemID )
						{
							return true;
						}
					}
...
#define AmuletOfSimplicity	700138
#define AmuletOfEase		700137

uint32 ItemInterface::GetSimplicityMod()
{
	if( IsEquipped( AmuletOfSimplicity ) )
		return 10;
	else if( IsEquipped( AmuletOfEase ) )
		return 5;
	else
		return 0;
}

uint32 GetSimplicityModFromItem( uint32 ItemId )
{
	switch( ItemId )
	{
	case AmuletOfSimplicity:
		return 10;
	case AmuletOfEase:
		return 5;
	default:
...
#undef AmuletOfSimplicity
#undef AmuletOfEase

bool ItemInterface::CanSwapSimplicityItem( Item* srcItem, Item*  dstItem )
{
	Player * owner = GetOwner();
	if( srcItem == NULL || owner == NULL || owner->GetSession() == NULL )
		return false;

	uint32 srcItemSimp = GetSimplicityModFromItem( srcItem->GetEntry() );
	if( !srcItemSimp ) // No simplicity item, so let's swap it!
		return true;

	bool found = false;
	uint32 toEquip = 0;
	if( IsEquipped( srcItem->GetEntry() ) )
	{
		if( dstItem != NULL )
			toEquip = dstItem->GetEntry();
	}
	else if( dstItem == NULL || IsEquipped( dstItem->GetEntry() ) )
		toEquip = srcItem->GetEntry();

	uint32 toEquipSimp = GetSimplicityModFromItem( toEquip );
	for( uint32 x = EQUIPMENT_SLOT_START; x < EQUIPMENT_SLOT_END; ++x )
	{
		Item * it = m_pItems[x];
		if( it != NULL && it->GetProto() != NULL )
		{
			if( ( it->GetProto()->RequiredLevel - toEquipSimp ) > owner->getLevel() )
			{
				owner->GetSession()->SystemMessage("You can not unequip that right now. You need to unequip higher level items first!");
				return false;
			}
		}
	}
