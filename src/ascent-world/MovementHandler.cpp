			...
			if( recv_data.GetOpcode() == MSG_MOVE_FALL_LAND )
			{
				...
				if( _player->isAlive() && !_player->GodModeCheat && falldistance > 12 && ( UNIXTIME >= _player->m_fallDisabledUntil ) && movement_info.FallTime > 2000 )
				{
					...
					if(_player->m_stealth)
						_player->RemoveStealth(); // ceberwow : why again? lost stealth by AURA_INTERRUPT_ON_ANY_DAMAGE_TAKEN already. Egari: hush you nub.. double checking is gewd...
				}
