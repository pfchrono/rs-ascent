...
const char * gCreatureNameFormat						= "usssuuuuuuuuuuffccu";
...
const char * gQuestFormat								= "uuuuuuuuuuuuuuuuuuussssssssssuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuiiiiiiuiuuuuuffuuuuuuuuuuuuuuuuuuuu";
...
const char * gWorldMapInfoFormat						= "uuuuufffusuuuuuuuufuu";
const char * gZoneGuardsFormat							= "uuu";
const char * gUnitModelSizeFormat						= "uf";

const char * gForceFlyModeFormat						= "uu";
...
void ObjectMgr::LoadSpellSpellRelation()
{
	QueryResult * result = WorldDatabase.Query( "SELECT * from spell_spell_relation" );
	if ( result != NULL )
	{
		do
		{
			Field *f = result->Fetch();
			SpellSpellRelationTable.insert( pair< uint32, uint32 >( f[0].GetUInt32(), f[1].GetUInt32() ) );
		}
		while( result->NextRow() );
		delete result;
	}
...
bool Storage_ReloadTable(const char * TableName)
{
	...
	if(!stricmp(TableName, "creature_template"))
	{
	...
	else if(!stricmp(TableName, "spellfixes"))			// Spell Fixes
		objmgr.LoadSpellFixes();
	else if(!stricmp(TableName, "spell_disable"))		// Spell Disable
		objmgr.LoadDisabledSpells(true);
	else if(!stricmp(TableName, "vendors"))				// Vendors
