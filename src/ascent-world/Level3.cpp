...
//	uint32 guid = GUID_LOPART(m_session->GetPlayer()->GetSelection());
	...
	if(!crt) return true;
	uint32 guid = crt->GetLowGUID();
	if(crt->GetCreatureInfo())
		BlueSystemMessage(m_session, "Showing creature info for %s", crt->GetCreatureInfo()->Name);
	GreenSystemMessage(m_session, "GUID: %u", guid);
	GreenSystemMessage(m_session, "Faction: %u", crt->GetUInt32Value(UNIT_FIELD_FACTIONTEMPLATE));
	GreenSystemMessage(m_session, "NPC Flags: %u", crt->GetUInt32Value(UNIT_NPC_FLAGS));
	GreenSystemMessage(m_session, "Unit Flags: %u", crt->GetUInt32Value(UNIT_FIELD_FLAGS));
	GreenSystemMessage(m_session, "DisplayID: %u", crt->GetUInt32Value(UNIT_FIELD_DISPLAYID)); 
	...
	GreenSystemMessage(m_session, "Resists - H: %u - Fi: %u - N: %u - Fr: %u - S: %u - A: %u", 
		crt->GetUInt32Value(UNIT_FIELD_RESISTANCES_01), crt->GetUInt32Value(UNIT_FIELD_RESISTANCES_02), 
		crt->GetUInt32Value(UNIT_FIELD_RESISTANCES_03), crt->GetUInt32Value(UNIT_FIELD_RESISTANCES_04), 
		crt->GetUInt32Value(UNIT_FIELD_RESISTANCES_05), crt->GetUInt32Value(UNIT_FIELD_RESISTANCES_06));
...
bool ChatHandler::HandleFlyCommand(const char* args, WorldSession* m_session)
{
	...
	if( chr && chr->RS_getAccess() >= RS_GAMEMASTER )
...
bool ChatHandler::HandleFlySpeedCheatCommand(const char* args, WorldSession* m_session)
{
	...
	if(self->RS_getAccess() < RS_PROBEGM)
	{
		if( Speed > 15 || Speed < 1)
		{
			RedSystemMessage(m_session, "Invalid speed. Please enter a value between 1 and 15.");
			return true;
		}
		plr = self;
	}
...
bool ChatHandler::HandleModifyLevelCommand(const char* args, WorldSession* m_session)
{
	...
	if( Level == 0 || Level > 255 || ( Level > sWorld.m_levelCap && plr->RS_getAccess() < RS_PROBEGM && m_session->GetPlayer()->RS_getAccess() <= RS_SCRIPTER ) )
	{
...
*/
	RedSystemMessage(m_session, "Command disabled.");
	return true;
}
...
bool ChatHandler::HandleAddPetSpellCommand(const char* args, WorldSession* m_session)
{
	...
	GreenSystemMessage(m_session, "Added spell %u to the pet.", SpellId);
	return true;
}
...
bool ChatHandler::HandleRemovePetSpellCommand(const char* args, WorldSession* m_session)
{
	...
	GreenSystemMessage(m_session, "Removed spell %u from the pet.", SpellId);
	return true;
}
...
bool ChatHandler::HandleRenamePetCommand(const char* args, WorldSession* m_session)
{
	...
	GreenSystemMessage(m_session, "Renamed pet %s to %s.", pPet->GetName(), args);
	pPet->Rename(args);
	return true;
}
...
bool ChatHandler::HandlePlayerInfo(const char* args, WorldSession * m_session)
{
	...
	if(!plr){
		RedSystemMessage(m_session, "Unable to locate player.");
		return true;
	}

	uint32 toMatch = m_session->GetPlayer()->RS_getAccess() - 4;
	if( toMatch < 0 )
		toMatch = 0;

	if( plr->m_isGmInvisible > 0 && plr->m_isGmInvisible >= toMatch )
	{
		RedSystemMessage(m_session, "Unable to locate player.");
		return true;
	}
...
bool ChatHandler::HandleRemoveItemCommand(const char * args, WorldSession * m_session)
{
	...
	if( iProto )
	{
		...
		BlueSystemMessage(m_session, "Invalid item with ID: %u", item_id);
...
bool ChatHandler::HandleUnStuckCommand(const char * args,WorldSession * m_session)
{
	...
	PlayerInfo * info = objmgr.GetPlayerInfoByName(args);
	if(info)
	{
		Player * plr = objmgr.GetPlayer(info->guid);
		if(plr && plr->GetSession())
		{
			if(!plr->RemoveTransporterStatus())
			{
				char buf[256];
				snprintf((char*)buf,256, "%s already being teleported.", plr->GetName());
				SystemMessage(m_session, buf);
				return true;
			}
		...
		{
			CharacterDatabase.Execute("UPDATE characters SET positionX=bindpositionX, positionY=bindpositionY, positionZ=bindpositionZ, mapId=bindmapId, zoneId=bindzoneId WHERE name='%s'", info->name);
		}
	}
	else
	{
		RedSystemMessage(m_session, "There is no such player.");
		return true;
	}
	GreenSystemMessage(m_session, "Forcing %s to rezone to bind location.", args);
	return true;
}
...
void SendItemLinkToPlayer(ItemPrototype * iProto, WorldSession * pSession, uint32 ItemCount = 0, Player * owner = NULL, uint32 language = NULL)
{
	if(!iProto || !pSession)
		return;
	if(ItemCount && owner == NULL)
 	...
 	if(ItemCount > 0)
		sChatHandler.SystemMessage(pSession,"Item %u %s  count %u", iProto->ItemId, GetItemLinkByProto(iProto, language), ItemCount);
 	else
...
bool ChatHandler::HandleLookupItemCommand(const char * args, WorldSession * m_session)
{
	...
	while(!itr->AtEnd())
	{
		...
		if(FindXinYString(x, it->lowercase_name) || localizedFound)
		{
			...
			SendItemLinkToPlayer(it, m_session, 0, 0, localizedFound ? m_session->language : 0);
			++count;
			if(count == 25)
			{
...
bool ChatHandler::HandleLookupSpellCommand(const char * args, WorldSession * m_session)
{
	...
	bool bShowTrainer = true;
	bool bShowNonTrainer = true;
	if(x.length() < 4)
	{
		RedSystemMessage(m_session, "Your search string must be at least 4 characters long.");
		return true;
	}
	BlueSystemMessage(m_session, "Starting search of spells `%s`...", x.c_str());

	GreenSystemMessage(m_session, "Spell matches: id: name");
	...
	for(uint32 i = 0; i <= dbcSpell.GetNumRows(); ++i)
	{
		...
		if(sp)
		{
			if((sp->Id) && (sp->Name))
			{
				string y = string(sp->Name);
				ASCENT_TOLOWER(y);
				
				if(y.find(x) != std::string::npos)
				{
					string spId = MyConvertToString(sp->Id);
					while(spId.length() < 8) 
					{
						spId = ' ' + spId;
					}
					string spRank = "";
					if(sp->Rank)	
						spRank = string(sp->Rank);
	
					if(sp->RankNumber)
						spRank = MyConvertToString(sp->RankNumber);

					string spDescr = "";
					if(sp->Description)
						spDescr = string(sp->Description);

					string spName = "";
					spName = string(sp->Name);

					if(spRank.length() > 0)
						spName = spName + " (Rank " + spRank + ")";

					if(bShowTrainer)
					{
						//is this a trainer spell?
						QueryResult * result = WorldDatabase.Query("SELECT entry FROM trainer_spells where cast_spell = '%u'",sp->Id);
						if(result)
						{
							spName = spName + " [TRAINER]";
							delete result;

							ok2Show = true;
						}
						else
							ok2Show = (bShowNonTrainer == true);
					}
					else
					{
						if(bShowNonTrainer == true)
							ok2Show = true;
					}

					tmpFind = spDescr;
					ASCENT_TOLOWER(tmpFind);
					if(tmpFind.find("teaches") != std::string::npos)
					{
						if(spName.find(" [TRAINER]") != std::string::npos)
							spName = spName + " [TRAINER]";

						if(bShowTrainer == false)
							ok2Show = false;
					}	

					if((bShowTrainer == true) && (bShowNonTrainer == true))
						ok2Show = true;

					if(ok2Show == true)
					{
						GreenSystemMessage(m_session, "%s: %s", spId.c_str(), spName.c_str());
						if(spDescr.length() > 0)
							SystemMessage(m_session, " (%s)", spDescr.c_str());
	...
	if (count == 0)
		RedSystemMessage(m_session, "No matches found; search completed in %u ms.", getMSTime() - t);
	else
		BlueSystemMessage(m_session, "Search completed in %u ms.", getMSTime() - t);
...
bool ChatHandler::HandleShowItems(const char * args, WorldSession * m_session)
{
	...
	for(; !itr.End(); itr.Increment())
	{
		...
		SendItemLinkToPlayer((*itr)->GetProto(), m_session, (*itr)->GetUInt32Value(ITEM_FIELD_STACK_COUNT), plr, m_session->language);
	}
	itr.EndSearch();
