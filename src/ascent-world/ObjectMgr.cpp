...
ObjectMgr::~ObjectMgr()
{
	...
	// Remove bug-reports from memory.
	sBugReporter.Destroy();
...
void ObjectMgr::LoadInstanceBossInfos()
{
	...
	do
	{
		...
		if(m_InstanceBossInfoMap[bossInfo->mapid] == NULL)
			m_InstanceBossInfoMap[bossInfo->mapid] = new InstanceBossInfoMap;
		m_InstanceBossInfoMap[bossInfo->mapid]->insert(InstanceBossInfoMap::value_type(bossInfo->creatureid, bossInfo));
		cnt++;
	} while(result->NextRow());
...
bool ObjectMgr::HasExtendedCost( uint32 itemid )
{
	std::set<ExtendedCostItem*>::iterator itr = ExtendedCostItemTable.begin();
	for ( ; itr != ExtendedCostItemTable.end(); itr++ )
	{
		ExtendedCostItem * eci = ( *itr );
		if ( eci != NULL && itemid == eci->itemid )
			return true;
	}
	return false;
}
...
void ObjectMgr::LoadVendors( )
{
	...
	if( result != NULL )
	{
		...
		do
		{
			...
			itr = mVendors.find( vendorid );
			if( itr == mVendors.end() )
			{
				items = new std::vector<CreatureItem>;
				mVendors[vendorid] = items;
			}
			else
			{
				items = itr->second;
			...
			if( ec != 0 )
			{
				...
				ExtendedCostItem * ECI = new ExtendedCostItem;
				ECI->itemid		=	itm.itemid;
				ECI->vendorid	=	vendorid;
				ECI->costid		=	ec;
				ExtendedCostItemTable.insert(ECI);
			}

			/*count++;
			if( count == 1000 )
			{
				sLog.outError("Vendor %u contains over 1000 items which may cause client errors! Please fix ASAP.", fields[0].GetUInt32());
				break;
			}*/
			items->push_back( itm );
		}
	...
	if(result)
	{
		if( result->GetFieldCount() != 8 )
		{
			Log.LargeErrorMessage(LARGERRORMESSAGE_WARNING, "Incorrect column count at spellfixes, skipping, please fix it.",
				"Rockscent has skipped loading this table in order to avoid crashing.", NULL);
			return;
		}
...
void ObjectMgr::LoadDisabledSpells(bool reload)
{
	if( reload && !m_disabled_spells.empty() )
...
void ObjectMgr::SafeDeleteObj(WorldPacket *data)
{
	if( data )
	{
		delete data;
...
void ObjectMgr::SafeDeleteObj(Object *pObj)
{
	if( pObj )
	{
		delete pObj;
