...
/*
	...
	while ((packet = _recvQueue.Pop()))
	{
		...
		else
		{
			...
			if(Handler->status == STATUS_LOGGEDIN && !_player && Handler->handler != 0 && packet->GetOpcode() != CMSG_NAME_QUERY)
			{
...
bool WorldSession::CanUseCommand(char cmdstr)
{
	...
	if(cmdstr == 0)
		return true;
	if(permissions[0] == 'a' && cmdstr != 'z')	// GameMasters (+admin)
		return true;
	if(permissions[0] == 'b' && cmdstr != 'a' && cmdstr != 'c' && cmdstr != 'z') // Scripters
		return true;
	if(permissions[0] == 'c' && cmdstr != 'a' && cmdstr != 'b' && cmdstr != 'z') // Probe GMs
		return true;
...
//	WorldPacketHandlers[CMSG_MOVE_FALL_RESET].handler							= &WorldSession::HandleMoveFallResetOpcode;
	...
	WorldPacketHandlers[MSG_QUERY_GUILD_BANK_TEXT].handler						= &WorldSession::HandleGuildQueryBankTabInfo;
	WorldPacketHandlers[CMSG_SET_GUILD_BANK_TEXT].handler						= &WorldSession::HandleGuildSetBankTabInfo;
	...
	WorldPacketHandlers[CMSG_REPORT_PVP_AFK].handler							= &WorldSession::HandleReportPlayerAFKOpcode;
	...
	WorldPacketHandlers[CMSG_SET_TITLE].handler									= &WorldSession::HandleChooseTitleOpcode;
...
void SessionLogWriter::writefromconsole(const char* format, ...)
{
	if(!IsOpen()) return;
	// Argument list
	va_list ap;
	...
	snprintf(out, 32768, "[%-4d-%02d-%02d %02d:%02d:%02d] ",aTm->tm_year+1900,aTm->tm_mon+1,aTm->tm_mday,aTm->tm_hour,aTm->tm_min,aTm->tm_sec);
	size_t l = strlen(out);
	// Add <Console> tag instead of account info :P
	snprintf(&out[l], 32768 - l, "<Console> :: ");
	// Build, send message and end list.
	l = strlen(out);
	vsnprintf(&out[l], 32768 - l, format, ap);
	fprintf(m_file, "%s\n", out);
	fflush(m_file);
