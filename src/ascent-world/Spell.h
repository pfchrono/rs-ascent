...
//	SMT_SPEED				=12,// movement speed, while given spell is active(flat is %) // TODO CHECK! ok this is not speed and is used with 23 misc to sometimes so is odd
	...
	SMT_AMPTITUDE			=19,// delay for nova, redirection time bonus for totem,maybe smth else // GOOD need work
...
enum SUMMON_TYPE
{
	...
	SUMMON_TYPE_UNK1			= 181,
	SUMMON_TYPE_UNK2			= 187,
	SUMMON_TYPE_UNK3			= 247,
	SUMMON_TYPE_CRITTER2		= 307,
	SUMMON_TYPE_CRITTER3		= 407,
	SUMMON_TYPE_UNK4			= 409,
	SUMMON_TYPE_UNK5			= 427,
	SUMMON_TYPE_POSSESSED2		= 428
};
...
ASCENT_INLINE bool TargetTypeCheck(Object *obj,uint32 ReqCreatureTypeMask)
{
	...
	if( obj->GetTypeId() == TYPEID_UNIT )
	{
		...
		return false;
	else return false;	//	omg, how in the heck did we cast it on a GO ? But who cares ?
	return true;
}
...
//	void SendHealManaSpellOnPlayer(Object * caster, Object * target, uint32 dmg, uint32 powertype);
	...
	uint8 CanTame( Unit * tgt, Player * p_caster);

	void HandleAuraAdd(uint64 guid);
	...
	void SummonWild(uint32 i);
	void SummonGuardian(uint32 i);
	void SummonDemon(uint32 i);
	void SummonCritter(uint32 i);
	void SummonPossessed(uint32 i);
	void CreateSummonTotem(uint32 i);
	void SummonGeneric(uint32 i);
	...
	ASCENT_INLINE uint32 GetType() { return ( GetProto()->Spell_Dmg_Type == SPELL_DMG_TYPE_NONE ? SPELL_DMG_TYPE_MAGIC : m_spellInfo->Spell_Dmg_Type ); }
