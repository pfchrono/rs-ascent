...
CreatureAIScript* ScriptMgr::CreateAIScriptClassForEntry(Creature* pCreature)
{
	...
	{
#ifdef EGARI_DEBUG
		sLog.outDebug("Invalid Creature AI Ptr for ID: %u!", pCreature->GetEntry());
#endif
		return NULL;
	}
...
{
	if( pPlayer->GetMapId() == 13 && pPlayer->RS_getAccess() < RS_PROBEGM )
	{
		pPlayer->GetSession()->SendNotification("You cannot use that here!");
		return true;
	}
...
{
	if(sEventMgr.HasEvent(_unit, EVENT_SCRIPT_UPDATE_EVENT))
...
void GossipScript::GossipHello(Object* pObject, Player* Plr, bool AutoSend)
{
	...
	if ( pCreature->IsSpiritHealer() == true ) //Sprit Healer should not have a gossip menu
	{
	...
	if(flags & UNIT_NPC_FLAG_VENDOR)
		Menu->AddItem(GOSSIP_ICON_VENDOR, "I would like to browse your goods", 1);
	...
	if((flags & UNIT_NPC_FLAG_TRAINER || flags & UNIT_NPC_FLAG_TRAINER_PROF || flags & UNIT_NPC_FLAG_TRAINER_CLASS) && pTrainer != 0)
	{
		...
		if(pTrainer->TrainerType != TRAINER_TYPE_PET)
		{
			...
			if(pTrainer->RequiredClass)
			{
				...
				Menu->AddItem(GOSSIP_ICON_TRAINER, msg.c_str(), 2);
			}
			else
			{
				msg += "training, ";
		...
		{
			Menu->AddItem(GOSSIP_ICON_TRAINER, "Train me in the ways of the beast.", 2);
		}
	}

	if(flags & UNIT_NPC_FLAG_TAXIVENDOR)
		Menu->AddItem(GOSSIP_ICON_TAXI, "Give me a ride.", 3);

	if(flags & UNIT_NPC_FLAG_AUCTIONEER)
		Menu->AddItem(GOSSIP_ICON_TXTBALLOON, "I would like to make a bid.", 4);

	if(flags & UNIT_NPC_FLAG_INNKEEPER)
		Menu->AddItem(GOSSIP_ICON_INNKEEPER, "Make this inn your home.", 5);

	if(flags & UNIT_NPC_FLAG_BANKER)
		Menu->AddItem(GOSSIP_ICON_TXTBALLOON, "I would like to check my deposit box.", 6);

	/*if(flags & UNIT_NPC_FLAG_SPIRITHEALER) 
		Menu->AddItem(GOSSIP_ICON_TXTBALLOON, "Bring me back to life", 7);*/

	if(flags & UNIT_NPC_FLAG_ARENACHARTER)
		Menu->AddItem(GOSSIP_ICON_TXTBALLOON, "How do I create a guild/arena team?", 8);

	if(flags & UNIT_NPC_FLAG_TABARDCHANGER)
		Menu->AddItem(GOSSIP_ICON_TXTBALLOON, "I want to create a guild crest.", 9);

	if(flags & UNIT_NPC_FLAG_BATTLEFIELDPERSON)
		Menu->AddItem(GOSSIP_ICON_TXTBALLOON, "I would like to go to the battleground.", 10);
...
void GossipScript::GossipSelectOption(Object* pObject, Player* Plr, uint32 Id, uint32 IntId, const char * EnteredCode)
{
	...
	case 11:
		// switch to talent reset message
		{
			...
			Menu->AddItem(GOSSIP_ICON_TXTBALLOON, "I understand, continue.", 12);
			Menu->SendTo(Plr);
		}break;
	case 12:
		// talent reset
...
void HookInterface::OnTitleEarned(CharTitlesEntry * ct, Player * pPlayer)
{
	OUTER_LOOP_BEGIN(SERVER_HOOK_EVENT_ON_TITLE_EARNED, tOnTitleEarned)
		(call)(ct, pPlayer);
	OUTER_LOOP_END
}
