...
void Aura::AddMod( uint32 t, int32 a, uint32 miscValue, uint32 i )
{
	...
	if( m_modcount >= 3 )
	{
		sLog.outDebug("Tried to add >3 (%u) mods to spellid %u [%u:%u, %u:%u, %u:%u]", m_modcount+1, this->m_spellProto->Id, m_modList[0].m_type, m_modList[0].m_amount, m_modList[1].m_type, m_modList[1].m_amount, m_modList[2].m_type, m_modList[2].m_amount);
		return;
	}
...
void Aura::AddAuraVisual()
{
	...
	if (caster != NULL && caster->IsPlayer() && caster->IsInWorld())
	{
		...
		data2 << m_target->GetNewGUID() << m_visualSlot << uint32(m_spellProto->Id) << uint32(m_duration) << uint32(m_duration);
...
void Aura::SpellAuraModStun(bool apply)
{
	...
	else if( (m_flags & (1 << mod->i)) == 0 ) //add these checks to mods where imunity can cancel only 1 mod and not whole spell
	{
		...
		if(GetCaster() && GetCaster()->IsUnit())
		{
			...
			if(u_caster->GetUInt32Value(UNIT_FIELD_CHANNEL_OBJECT))
			{
				if(u_caster->GetCurrentSpell() != NULL)
					u_caster->CancelSpell(u_caster->GetCurrentSpell());
			}
		}
...
void Aura::SpellAuraPeriodicTriggerSpell(bool apply)
{
	...
	if( !apply && m_target && m_target->IsPlayer() && ( GetSpellId() == BG_IDLE || GetSpellId() == BG_INACTIVE ) )
		static_cast<Player*>(m_target)->ClearAFKReporterList();		

	if(IsPassive() && !(m_target->IsCreature() && static_cast <Creature*> (m_target)->IsTotem()) && m_spellProto->spellIconID != 2010  && m_spellProto->spellIconID != 2020 && m_spellProto->spellIconID != 2255) //this spells are passive and are not done on the attack...
	{
...
void Aura::SpellAuraModSchoolImmunity(bool apply)
{
	...
	if(apply)
	{
		if( m_spellProto->NameHash == SPELL_HASH_DIVINE_SHIELD || m_spellProto->NameHash == SPELL_HASH_BLESSING_OF_PROTECTION || m_spellProto->NameHash == SPELL_HASH_ICE_BLOCK )
		{
			m_target->RemoveAurasByInterruptFlag(AURA_INTERRUPT_ON_INVINCIBLE);
		}
		...
		if(m_caster->IsPlayer())
		{
			...
			if(m_spellProto->NameHash == SPELL_HASH_DRAIN_LIFE)
			{
				...
				if(HasSoulSiphon)
				{
					...
					{
						Aura * pAura = m_target->m_auras[i];
						if( pAura && pAura->GetSpellId() )
						{
							skilllinespell * sks = objmgr.GetSpellSkill(pAura->GetSpellId());
							if(sks && sks->Id == SKILL_AFFLICTION)
							{
								AffCount++;
								TotalSSPer += PerAff;
								if(AffCount >= MaxAff)
...
void Aura::SpellAuraTransform(bool apply)
{
	if( mod == NULL )
	{
		sLog.outDebug("Aura::SpellAuraTransform | mod = NULL | Spell: %u", GetSpellId());
		return;
	}
	// Try a dummy SpellHandler
	if(sScriptMgr.CallScriptedDummyAura(GetSpellId(), mod->i, this, apply))
	...
	Unit * target = p_target?p_target:m_target;
	if( target && 
		target->GetUInt32Value(UNIT_FIELD_DISPLAYID)!= target->GetUInt32Value(UNIT_FIELD_NATIVEDISPLAYID)
		)
	{
		target->RemoveAllAuraType(SPELL_AURA_TRANSFORM);
		target->RemoveAllAuraType(SPELL_AURA_MOD_SHAPESHIFT);
	}
	...
	switch( GetSpellProto()->Id )
	{
		...
		case 47057: // Fiery Seduction
			{
				if(apply)
					m_target->SetUInt32Value(UNIT_FIELD_DISPLAYID, (RandomUInt(2)-1)?23523:23732);
				else
					m_target->SetUInt32Value(UNIT_FIELD_DISPLAYID, m_target->GetUInt32Value( UNIT_FIELD_NATIVEDISPLAYID ) );
			}break;
...
void MountedAuraAdditional(bool apply, Player * p_target, Aura *pAura)
{
	if( pAura == NULL || p_target == NULL )
		return;

	if( p_target->RS_getAccess() < RS_GAMEMASTER )
		return;

	uint32 flySpeedModifier, runSpeedModifier = 0;
	bool mOverride = false;
	switch(pAura->GetSpellId())
	{
	case 24576:
		{
			runSpeedModifier = 200;
			flySpeedModifier = 500;
			mOverride = true;
		}break;
	case 32420:
		{
			flySpeedModifier = 133;
			mOverride = true;
		}break;
	}
	if(mOverride)
	{
		if(apply)
		{
			p_target->EnableFlight();
			if(flySpeedModifier > 0)
				p_target->m_flyspeedModifier += flySpeedModifier;
			if(runSpeedModifier > 0)
				p_target->m_runSpeed += runSpeedModifier;
		}
		else
		{
			p_target->DisableFlight();
			if(flySpeedModifier > 0)
				p_target->m_flyspeedModifier -= flySpeedModifier;
			if(runSpeedModifier > 0)
				p_target->m_runSpeed -= runSpeedModifier;
		}
		p_target->UpdateSpeed();
	}
}
...
void Aura::SpellAuraMounted(bool apply)
{
	...
	MountedAuraAdditional(apply, p_target, this);
...
void Aura::EventJumpAndHeal()
{
	...
	s->Init(caster, GetProto(), true, NULL);
	...
	uint32 stacks=m_stackcount;

	if (Targets.size() == 0 || stacks == 0)
		Remove(true);
	else
	{
		//jump
		...
		s->forced_applications=stacks-1;
		s->prepare(&t);
		Remove(true);
	}
	*/
...
{
/*	//we add proccharges ontop, and limit to maxstack
	...
	if (m_spellProto->maxstack != 0)
		max = m_spellProto->maxstack;
	...
	if (GetDuration() > 0)
	{
		sEventMgr.ModifyEventTimeLeft(this, EVENT_AURA_REMOVE, GetDuration() + 200);

		if (m_target->IsPlayer())
		{
			WorldPacket data(SMSG_UPDATE_AURA_DURATION, 5);
			data << m_visualSlot << (uint32)GetDuration();
			static_cast<Player*>(m_target)->GetSession()->SendPacket(&data);
		}

		WorldPacket data2(SMSG_SET_EXTRA_AURA_INFO_2, 22);
		data2 << m_target->GetNewGUID() << m_visualSlot << uint32(m_spellProto->Id) << int32(-1) << int32(-1);
		...
		{
			WorldPacket data3(SMSG_SET_EXTRA_AURA_INFO_2, 22);
			data3 << m_target->GetNewGUID() << m_visualSlot << uint32(m_spellProto->Id) << uint32(GetDuration()) << uint32(GetDuration());
