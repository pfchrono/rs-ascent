...
void WorldSession::SendBattlegroundList(Creature* pCreature, uint32 mapid)
{
	...
	if (mapid == 0)
	{
		if( pCreature->GetCreatureInfo() )
		{
			...
			else if(strstr(pCreature->GetCreatureInfo()->SubName, "Eye of") != NULL)
...
void WorldSession::HandleReportPlayerAFKOpcode(WorldPacket &recv_data)
{
	CHECK_INWORLD_RETURN
	CHECK_PACKET_SIZE( recv_data, 8);
	...
	if( plr == NULL || plr->GetMapMgr() == NULL ) return;
	Player * Target = plr->GetMapMgr()->GetPlayer( guid );
	if( Target == NULL || !Target->IsInWorld() ) return;

	bool valid = !( Target->IsInAFKReporterList( plr->GetLowGUID() ) );
	if( valid )
	{
		if( Target->HasFlag(PLAYER_FLAGS, PLAYER_FLAG_AFK) )
		{
			if( !Target->AddToAFKReporterList( plr->GetLowGUID() ) )
				return;

			Target->m_amtReportedAFK++;

			if( Target->m_amtReportedAFK >= 3 )
				Target->CastSpell(Target, 43680, true);
	
			SystemMessage("You have succesfully reported %s for being AFK.", Target->GetName());
		}
		else
			SystemMessage("Failed! You tried to report a player that is not tagged as AFK!");
	}
	else
		SystemMessage("Failed! You have already reported this player!");
}