...
void WorldSession::HandleChooseTitleOpcode( WorldPacket & recv_data )
{
	...
	if(title > 0)
	{
		if(!GetPlayer()->HasFlag64(PLAYER_FIELD_KNOWN_PVPTITLES,uint64(1) << title) 
			&& !GetPlayer()->HasFlag64(PLAYER_FIELD_KNOWN_PVETITLES,uint64(1) << title))
			title = 0;
	}
	else
		title = 0;
...
{
	if( pPlayer == NULL 
		|| pPlayer->GetMapId() == 559 
		|| pPlayer->GetMapId() == 562 
		|| pPlayer->GetMapId() == 572
		)
		return;
