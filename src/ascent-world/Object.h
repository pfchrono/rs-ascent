...
#define GUID_HIPART2(x)					( uint32 )( (uint64(x) >> 48) & 0x0000FFFF )

#define GET_TYPE_FROM_GUID(x)			( GUID_HIPART((x)) & HIGHGUID_TYPE_MASK )
#define GET_LOWGUID_PART(x)				( GUID_LOPART((x)) & LOWGUID_ENTRY_MASK )

#define IS_CREATURE_GUID(Guid)			( GUID_HIPART2(Guid) == HIGHGUID_TYPE_UNIT )
#define IS_PET_GUID(Guid)				( GUID_HIPART2(Guid) == HIGHGUID_TYPE_PET )
#define IS_CREATURE_OR_PET_GUID(Guid)	( IS_CREATURE_GUID(Guid) || IS_PET_GUID(Guid) )
#define IS_PLAYER_GUID(Guid)			( GUID_HIPART2(Guid) == HIGHGUID_TYPE_PLAYER && Guid != 0 )
#define IS_UNIT_GUID(Guid)				( IS_CREATURE_OR_PET_GUID(Guid) || IS_PLAYER_GUID(Guid) )

#define IS_ITEM_GUID(Guid)				( GUID_HIPART2(Guid) == HIGHGUID_TYPE_ITEM )
#define IS_GAMEOBJECT_GUID(Guid)		( GUID_HIPART2(Guid) == HIGHGUID_TYPE_GAMEOBJECT )
#define IS_DYNAMICOBJECT_GUID(Guid)		( GUID_HIPART2(Guid) == HIGHGUID_TYPE_DYNAMICOBJECT )
#define IS_CORPSE_GUID(Guid)			( GUID_HIPART2(Guid) == HIGHGUID_TYPE_CORPSE )
#define IS_TRANSPORT(Guid)				( GUID_HIPART2(Guid) == HIGHGUID_TYPE_TRANSPORT )
#define IS_MO_TRANSPORT(Guid)			( GUID_HIPART2(Guid) == HIGHGUID_TYPE_MO_TRANSPORT )
...
/*		uint64 entry = *(uint64*)m_uint32Values;
	...
	ASCENT_INLINE bool HasMask(uint32 Field, uint32 Mask)
	{
		if(Field & Mask)
			return true;
		else
	...
	ASCENT_INLINE void SetFlag64( uint32 index, uint64 newFlag )
	{
	...
	ASCENT_INLINE void RemoveFlag64( uint32 index, uint64 oldFlag )
	{
		uint64 oldval = GetUInt64Value( index );
	...
	ASCENT_INLINE void ToggleFlag64( uint32 index, uint64 flag)
	{
	...
	ASCENT_INLINE bool HasFlag64( uint32 index, uint64 flag ) const
	{
		ASSERT( index < m_valuesCount || PrintIndexError( index , false ) );
		return (GetUInt64Value( index ) & flag) != 0;
	}

	ASCENT_INLINE void ApplyModFlag64( uint16 index, uint64 flag, bool apply)
	{
	...
	{
		if( pObj && pObj->GetTypeId() == TYPEID_PLAYER )
	...
	void SendAttackerStateUpdate( Object* Caster, Object* Target, dealdamage *Dmg, uint32 Damage, uint32 Abs, uint32 BlockedDamage, uint32 HitStatus, uint32 VStage );
