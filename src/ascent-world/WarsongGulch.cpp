...
void WarsongGulch::HookOnAreaTrigger(Player * plr, uint32 id)
{
	...
	if(((id == 3646 && plr->GetTeam() == 0) || (id == 3647 && plr->GetTeam() == 1)) && (plr->m_bgHasFlag && m_flagHolders[plr->GetTeam()] == plr->GetLowGUID()))
	{
		...
		{
			if( plr->HasAura( BG_INACTIVE ) )
				continue;

			plr->m_bgScore.BonusHonor += flagHonorTable[m_lgroup];
			HonorHandler::AddHonorPointsToPlayer(plr, flagHonorTable[m_lgroup]);
		}
		...
		if(m_scores[plr->GetTeam()] == 3)
		{
			...
			for(uint32 i = 0; i < 2; ++i)
			{
				for(set<Player*>::iterator itr = m_players[i].begin(); itr != m_players[i].end(); ++itr)
				{
					...
					if( (*itr)->HasAura( BG_INACTIVE ) || (*itr)->HasFlag(PLAYER_FLAGS, PLAYER_FLAG_AFK) )
						continue;
