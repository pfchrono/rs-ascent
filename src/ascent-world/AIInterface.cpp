...
{
	if( !m_Unit || m_AIState == STATE_EVADE || m_fleeTimer != 0 || !pUnit || !pUnit->isAlive() || m_Unit->IsPacified() || m_Unit->IsFeared() || m_Unit->IsStunned() || !m_Unit->isAlive() || !m_AllowedToEnterCombat )
	{
		return;
	}
	...
	if( m_Unit->isDead() )
	{
		m_canMove = false;
		m_AIState = STATE_IDLE;
		HandleEvent(EVENT_LEAVECOMBAT, pUnit, 0);
		return;
	}
...
//	If m_Unit is Pet then check owner hostility towards pUnit.
	if( m_Unit->IsPet() 
		&& static_cast<Pet*>(m_Unit)->GetPetOwner()
		&& static_cast<Pet*>(m_Unit)->GetPetOwner()->IsPlayer() )
	{
		Player * PetOwner = static_cast<Pet*>(m_Unit)->GetPetOwner();
		if(!isHostile( PetOwner, pUnit ) )
		{
			return false;
		}
	}
...
//	Object *pObj;
	...
	// FP guyz don't aggro on anyone...
	if(m_Unit->GetUInt32Value(UNIT_NPC_FLAGS) & UNIT_NPC_FLAG_TAXIVENDOR)
		return 0;

	if( m_Unit != NULL
		&& !m_Unit->IsInInstance()
		&& !m_Unit->IsInBg()
		&& m_Unit->IsCreature() == true
		&& static_cast< Creature*>(m_Unit)->GetCreatureInfo()
		&& static_cast< Creature*>(m_Unit)->GetCreatureInfo()->GuardType >= GUARDTYPE_PVP 
		)
	{
		std::set<Player*>::iterator itr2, itr3;
		for( itr2 = m_Unit->GetInRangePlayerSetBegin(); itr2 != m_Unit->GetInRangePlayerSetEnd(); )
		{
			itr3 = itr2;
			++itr2;
			Player *pPlayer = (*itr3);
			Unit *ppUnit = static_cast< Unit* >( pPlayer );
			if(GuardInit(pPlayer) == true && pPlayer->IsInWorld() && !pPlayer->HasAura(2479) && !pPlayer->HasAura(46705) )
			{
				if(!ppUnit)
					continue;

				dist = m_Unit->GetDistanceSq(ppUnit->GetPositionX(), ppUnit->GetPositionY(), ppUnit->GetPositionZ());
				if(dist > distance)	 // we want to find the CLOSEST target
					continue;
		
				if( dist <= _CalcAggroRange(ppUnit) )
				{
		#ifdef LOS_CHECKS
					if( CollideInterface.CheckLOS( m_Unit->GetMapId( ), m_Unit->GetPositionNC( ), pPlayer->GetPositionNC( ) ) )
					{
						WorldPacket data(SMSG_AI_REACTION, 12);
						data << m_Unit->GetGUID() << uint32(2);
						static_cast< Player* >( pPlayer )->GetSession()->SendPacket( &data );
						return ppUnit;
					}
		#else
					WorldPacket data(SMSG_AI_REACTION, 12);
	...
	if( m_updateTargetsTimer2 < getMSTime() )
	{
		...
		for( itr2 = m_Unit->GetInRangeSetBegin(); itr2 != m_Unit->GetInRangeSetEnd(); )
		{
			...
			if(!isHostile(pUnit, m_Unit) || isSameFaction(pUnit, m_Unit))
				continue;
...
/*		if(m_isGuard)
		...
		if(target->IsPlayer())
		{
			Player * p_target = static_cast< Player* >( target );
			if( p_target && p_target->GetSession() )
			{
				WorldPacket data(SMSG_AI_REACTION, 12);
			...
			{
				sLog.outDebug("AIInterface:1842, p_target is NULL or p_target session is NULL");
			}
		}
...
bool AIInterface::FindFriends(float dist)
{
	...
	if ( m_Unit->IsCreature() == true )
	{
		if ( static_cast<Creature*>(m_Unit)->GetCreatureInfo()->GuardType >= GUARDTYPE_PVP )
			return NULL; //Peace keeper guards do not chain agro others around them.
	}
	if( m_Unit->GetUInt32Value(UNIT_FIELD_CREATEDBY) != 0 )
		return NULL; // Pets don't chain aggro.... Rawr...
...
/*	else
	{
		if ( m_Unit->GetCurrentSpell() != NULL )
			m_Unit->CancelSpell(m_Unit->GetCurrentSpell());
	}*/
...
{
	if( m_Unit && m_Unit->IsUnit() && isSameFaction(m_Unit, Attacker) )
		return 0;
...
bool AIInterface::GuardInit(Object * pObject)
{
	Creature * pCrt = (m_Unit->IsCreature())?((Creature*)m_Unit):NULL;
	Player * Plr = pObject->IsPlayer()?((Player*)pObject):NULL;
	if( Plr == NULL || pCrt == NULL || pCrt->GetCreatureInfo() == NULL )
		return false;

	uint8 guardType = pCrt->GetCreatureInfo()->GuardType;
	if( pCrt->IsGuard() )
	{
		switch(guardType)
		{
			case GUARDTYPE_PVP:
				{
					if( !Plr->HasAura(2479) && !Plr->HasAura(46705) )
						return Plr->m_pvpcombat;
					else
						return false;
				}break;
			case GUARDTYPE_VIP:
				{
					return ( !Plr->GetSession()->CanUseCommand('2') );
				}break;
			case GUARDTYPE_GM:
				{
					return ( !Plr->GetSession()->CanUseCommand('b') );
				}break;
			case GUARDTYPE_LEVEL:
				{
					return ( Plr->getLevel() <= pCrt->getLevel() );
				}break;
			default:
				{
					return false;
