...
void WorldSession::HandleLootMoneyOpcode( WorldPacket & recv_data )
{
	...
	uint32 curmoney = _player->GetUInt32Value(PLAYER_FIELD_COINAGE);
	uint32 money;
	if(_player->InGroup())
		money = pLoot->gold;
	else
		money = ( ( ( curmoney + pLoot->gold ) > GOLDLIMIT )?( GOLDLIMIT - curmoney ):( pLoot->gold ) );
	...
	else
	{
		...
		if(party)
		{
			...
			{
				curmoney = (*itr)->GetUInt32Value(PLAYER_FIELD_COINAGE);
				share = ( ( ( curmoney + share ) > GOLDLIMIT )?( GOLDLIMIT - curmoney ):( share ) );
...
void WorldSession::HandleWhoOpcode( WorldPacket & recv_data )
{
	...
	if( _player->RS_getAccess() >= RS_GAMEMASTER )
		gm = true;
	...
	while(itr !=iend && sent_count < 50)
	{
		...
		if(!sWorld.show_gm_in_who_list && !gm)
		{
			if(plr->RS_getAccess() >= RS_PROBEGM && ( plr->bGMTagOn || plr->m_isGmInvisible != INVIS_NONE ) )
				continue;
		}
...
void WorldSession::HandleLogoutRequestOpcode( WorldPacket & recv_data )
{
	...
	if(pPlayer)
	{
		...
		if(	HasGMPermissions() ) // or we are a gm
		{
...
void WorldSession::HandleBugOpcode( WorldPacket & recv_data )
{
	...
	{
		std::string msg;
		msg = "Type: " + type + ". " + content;
		CreateBugReport( msg.c_str() );
	}
}
...
void WorldSession::HandleSummonResponseOpcode(WorldPacket & recv_data)
{
	...
	if(!IsClickOk)
	{
		/*Player * plr = objmgr.GetPlayer( SummonerGUID );
		if(plr && plr->IsInWorld())
			plr->GetSession()->SystemMessage("%s declined your summoning request.", _player->GetName());*/
