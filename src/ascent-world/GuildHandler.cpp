...
void WorldSession::HandleGuildRank(WorldPacket & recv_data)
{
	...
	CharacterDatabase.Execute("REPLACE INTO guild_ranks VALUES(%u, %u, \"%s\", %u, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d)",
...
//--Confirmed as a 'crash-point'. Needs further testing before implementation.
//	pGuild->LogGuildEvent(GUILD_EVENT_TABARDCHANGE, 0);
//
	...
	else
	{
		...
		for(i = 0; i < pCharter->SignatureCount; ++i)
		{
			...
			if(info)
			{
				...
				{
					plr->GetSession()->SendArenaTeamCommandResult(ERR_ARENA_TEAM_FOUNDER_S, team->m_name, "", 0);
				}
			}
		}

		SendArenaTeamCommandResult(ERR_ARENA_TEAM_CREATE_S, team->m_name, "", 0);
...
void WorldSession::HandleGuildBankGetAvailableAmount(WorldPacket & recv_data)
{
	...
	if(_player->GetGuild() && _player->GetGuild()->GetGuildLeader() == _player->GetLowGUID())
		avail = money;
...
// Guild Bank Tab Text
void WorldSession::HandleGuildQueryBankTabInfo(WorldPacket & recv_data)
{
	uint8 TabId;
	recv_data >> TabId;

	GetPlayer()->GetGuild()->SendGuildBankTabInfo(this, TabId);
}

void WorldSession::HandleGuildSetBankTabInfo(WorldPacket & recv_data)
{
	uint8 TabId;
	recv_data >> TabId;
	string text;
	recv_data >> text;
	GetPlayer()->GetGuild()->SetGuildBankTabInfo(this, TabId, text);
}
