...
public:
	...
	uint32 GenerateRewardTitle( Quest * qst );
	void GenerateRewardItems( Player * plr, Quest * qst );
	...
	ASCENT_INLINE bool IsQuestRepeatable(Quest *qst) { return (qst->is_repeatable==QUEST_REPEATABLE?true:false); }
	ASCENT_INLINE bool IsQuestDaily(Quest *qst) { return (qst->is_repeatable==QUEST_REPEATABLE_DAILY?true:false); }
