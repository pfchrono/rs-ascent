...
public:
	...
	void RemoveVendorItem(uint32 itemid)
	{
		...
		for(std::vector<CreatureItem>::iterator itr = GetSellItemBegin(); itr != GetSellItemEnd(); ++itr)
		{
			if(itr->itemid == itemid)
			{
	...
	bool IsGuardType(uint8 GUARDTYPE){ return (GetCreatureInfo()->GuardType == GUARDTYPE)?true:false; }
	...
	ASCENT_INLINE void SetCreatureInfo(CreatureInfo *ci) { creature_info = ci; }
	...
	bool IsGuard()
	{
		if( GetCreatureInfo()->GuardType > GUARDTYPE_NONE )
			return true;
		else
			return false;
	};
