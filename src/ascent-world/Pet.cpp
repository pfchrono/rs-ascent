...
void Pet::CreateAsSummon(uint32 entry, CreatureInfo *ci, Creature* created_from_creature, Player *owner, SpellEntry* created_by_spell, uint32 type, uint32 expiretime, Spell* spell_callback)
{
	if(!ci || !owner) return;
...
void Pet::GiveXP( uint32 xp )
{
	...
	uint32 oLevel = m_Owner->getLevel();
	if( pLevel >= oLevel )		//pet do not get xp if its level >= owners level
		return;
...
void WorldSession::SendPetNameInvalid(uint32 error, std::string name)
{
...
bool hasThreeConsecutiveChars(string s_name)
{
	char name[1024];
	sprintf(name, s_name.c_str());
	
	uint32 size = s_name.size();
	uint32 i;
	for (  i = 0; i < size; ++i )
	{
		if( i != ( size - 2 ) )
...
bool WorldSession::isValidPetName(string NewName)
{
	uint32 error = 0;
	if( !NewName.size() )
		error = PET_NAME_NO_NAME;
	else
	{
		if( error == 0 && NewName.find("  ") != string::npos )
			error = PET_NAME_CONSECUTIVE_SPACES;
		if( error == 0 && NewName.find(" ") == 0 || NewName.rfind(" ") == 0 )
			error = PET_NAME_INVALID_SPACE;
		if( error == 0 && NewName.length() > 10 )
			error = PET_NAME_TOO_LONG;
		if( error == 0 && NewName.length() < 4 )
			error = PET_NAME_TOO_SHORT;
		if( error == 0 && hasThreeConsecutiveChars(NewName) )
			error = PET_NAME_THREE_CONSECUTIVE;
		if( error == 0 && !isAllAlpha(NewName) )
			error = PET_NAME_INVALID;

		if( error == 0 )
		{
			std::stringstream sqlquery;
			sqlquery << "SELECT COUNT(name) FROM banned_names WHERE name LIKE '%" << CharacterDatabase.EscapeString(NewName).c_str() << "%'";
			QueryResult * result = CharacterDatabase.Query(sqlquery.str().c_str());
			if(result)
			{
				if(result->Fetch()[0].GetUInt32() > 0)
					error = PET_NAME_RESERVED;
	...
	if( error != 0 )
	{
		SendPetNameInvalid(error, NewName);
		return false;
	}
	else
		return true;
