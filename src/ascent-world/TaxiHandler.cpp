...
void WorldSession::HandleActivateTaxiOpcode( WorldPacket & recv_data )
{
	...
	if( GetPlayer()->GetTeam() )
	{
		...
		*/
		...
		if( ci && taxinode->horde_mount > 0 ) // Only use id if valid...
			modelid = ci->Male_DisplayID;
		else
			modelid = 1566; // in case its a bad id...
	}
	else
	{
		/*
		...
		*/
		...
		if( ci && taxinode->alliance_mount > 0 )  // Only use id if valid...
			modelid = ci->Male_DisplayID;
		else
			modelid = 1147; // in case its a bad id...
	}
...
void WorldSession::HandleMultipleActivateTaxiOpcode(WorldPacket & recvPacket)
{
	...
	if( GetPlayer()->GetTeam() )
	{
		...
		if( taxinode->horde_mount == 2224 )
			...
			modelid =1566; // In case it's a bat or a bad id
		*/
		MountNPC = taxinode->horde_mount;
		if( MountNPC == 0 )
			MountNPC = taxinode->alliance_mount;

		CreatureInfo* ci = CreatureNameStorage.LookupEntry( MountNPC );
		if(ci)
			modelid = ci->Male_DisplayID;
		else
			modelid = 1566;
