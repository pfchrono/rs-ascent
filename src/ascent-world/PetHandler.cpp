...
void WorldSession::HandlePetAction(WorldPacket & recv_data)
{
			...
			case PET_ACTION_ATTACK:
				{
					...
					if( _player->m_CurrentCharm != pPet->GetGUID() && pPet != pTarget && RandomUInt(100) < 10 )
						pPet->SendPetTalk((uint32)PET_TALK_ATTACK);
					else
						pPet->SendPetAIReaction(targetguid);
	...
	pPet->SendPetTalk(PET_TALK_ATTACK);
}
...
{
	if( !GetPlayer()->IsInWorld() || !GetPlayer()->GetMapMgr() )
		return;
	...
	if( !sp )
	{
		sLog.outDebug("WORLD: Unknown PET spell id %u", spellid);
		sChatHandler.SystemMessage(this, "WORLD: unknown pet spell id %u", spellid);
		return;
	}

	Creature * pet = GetPlayer()->GetMapMgr()->GetCreature( uint32(guid) );
	if( !pet )
	{
		sLog.outDebug("WORLD: Pet %u not exist.", uint32(GUID_LOPART(guid)) );
		sChatHandler.SystemMessage(this, "WORLD: Couldn't find pet!");
		return;
	}

	if( pet != ((Creature*)GetPlayer()->GetSummon()) && pet->GetGUID() != GetPlayer()->m_CurrentCharm )
	{
		sLog.outError( "WORLD: Pet %u isn't pet of player %s", uint32(GUID_LOPART(guid)), GetPlayer()->GetName() );
		sChatHandler.SystemMessage(this, "WORLD: That pet does not belong to you!");
		return;
	}
	...
	if( !pet->RemoveAura( spellid ) )
	{
		sChatHandler.SystemMessage(this, "WORLD: Failed to cancel aura!");
		sLog.outDebug("PET SYSTEM: Player "I64FMT" failed to cancel aura %u from pet", _player->GetGUID(), spellid );
		pet->RemoveAurasDueToSpell( spellid ); // Try to force it this way?
	}
}
