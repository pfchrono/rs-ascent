...
void WorldSession::HandleSetTradeItem(WorldPacket & recv_data)
{
	...
	if(SourceSlot >= INVENTORY_SLOT_BAG_START && SourceSlot < INVENTORY_SLOT_BAG_END && TradeSlot < 6)
	{
		//More duping woohoo
		//sCheatLog.writefromsession(this, "tried to trade a container/bag");
		return;
	}
	...
	if( ownerguid == NULL || ownerguid != _player->GetGUID() )
	{
		...
		pItem->DeleteFromDB();
		sCheatLog.writefromsession(_player->GetSession(), "tried to trade an item that had owner==NULL, probable duper!");
		_player->GetItemInterface()->BuildInventoryChangeError(	pItem, NULL, INV_ERR_ITEM_NOT_FOUND);
		...
		else
		{
			...
			for(uint32 Index = 0; Index < 6; ++Index)
			{
				...
				if(Guid != 0)
				{
					...
					{
						if(GetPlayer()->RS_getAccess() >= RS_PROBEGM)
						{
			...
			if(pTarget->mTradeGold)
			{
				...
				uint32	TradeGold	=	pTarget->mTradeGold;

				if( curgold + pTarget->mTradeGold > GOLDLIMIT )
					TradeGold = GOLDLIMIT - curgold;

				_player->ModUnsigned32Value(PLAYER_FIELD_COINAGE, TradeGold);
				pTarget->ModUnsigned32Value(PLAYER_FIELD_COINAGE, -(int32)TradeGold);
			}
