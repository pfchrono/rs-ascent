...
uint32 QuestMgr::PlayerMeetsReqs(Player* plr, Quest* qst, bool skiplevelcheck)
{
	...
	else
	{
		...
		if( sQuestMgr.IsQuestDaily(qst) && ( plr->HasFinishedDaily(qst->id) || plr->GetFinishedDailyCount() >= 25 ) )
	...
	if(qst->required_access)
	{
		if(plr->RS_getAccess() < qst->required_access)
			return QMGR_QUEST_NOT_AVAILABLE;
	}
...
void QuestMgr::BuildOfferReward(WorldPacket *data, Quest* qst, Object* qst_giver, uint32 menutype, uint32 language, Player * plr)
{
	...
	*data << uint32( 0 );								// Honor Related
	*data << uint32( 0x08 );
	*data << uint32( qst->reward_spell );
	*data << uint32( qst->effect_on_player );
	*data << uint32( GenerateRewardTitle( qst ) );
	*data << uint32( 0 );
}
...
void QuestMgr::BuildQuestDetails(WorldPacket *data, Quest* qst, Object* qst_giver, uint32 menutype, uint32 language, Player * plr)
{
	...
	*data << uint32( GenerateRewardTitle( qst ) );	// CharTitleId, new 2.4.0, player gets this title (id from CharTitles)

	*data << uint32(1); // QUEST_EMOTE_COUNT
	*data << uint32( EMOTE_ONESHOT_TALK ); // DetailsEmote
	*data << uint32(0);	// DetailsEmoteDelay
/*	*data << uint32(1); // DetailsEmote
	*data << uint32(0);	// DetailsEmoteDelay
	*data << uint32(0); // DetailsEmote
	*data << uint32(0);	// DetailsEmoteDelay
	*data << uint32(0); // DetailsEmote
	*data << uint32(0);	// DetailsEmoteDelay*/
}
...
void QuestMgr::OnQuestFinished(Player* plr, Quest* qst, Object *qst_giver, uint32 reward_slot)
{
	...
	if( IsQuestDaily(qst) )
	{
		if (plr->m_finishedDailies.size() >= 25)
		{
			sChatHandler.SystemMessage(plr->GetSession(), "You cannot complete anymore daily quests today.");
			return;
		}

		if (plr->m_finishedDailies.find(qst->id) != plr->m_finishedDailies.end())
		{
	...
	if(IsQuestRepeatable(qst) || IsQuestDaily(qst) )
	{
		...
		if(qst->reward_title > 0)
		{
			CharTitlesEntry * ct = dbcCharTitle.LookupEntryForced(qst->reward_title);
			if( !ct )
			{
				plr->GetSession()->SystemMessage("There is an error in the title-reward set for this quest, please report on forums!");
			}
			else
			{
				 if( !plr->HasTitle(ct, TITLE_TYPE_PVE) )
					plr->AddTitle(ct, TITLE_TYPE_PVE );
				else
					plr->GetSession()->SystemMessage("You already have that title");
			}
		}
		...
		{
			plr->SetUInt32Value( PLAYER_FIELD_DAILY_QUESTS_00 + plr->GetFinishedDailyCount(), qst->id );
			plr->PushToFinishedDailies( qst->id );
//			if( plr->GetSession() != NULL )
//				plr->GetSession()->SystemMessage("You can only complete %u more daily quests today.", 25 - plr->GetFinishedDailyCount() );
		}
	}
	else
	{
		...
		uint32 dMONNIES = 0;
		uint32 MONNIES = plr->GetUInt32Value( PLAYER_FIELD_COINAGE );
		if(qst->reward_money != 0)
			dMONNIES = qst->reward_money;

		if(qst->reward_money < 0 && plr->GetUInt32Value( PLAYER_FIELD_COINAGE ) < uint32( 0 - dMONNIES ))
			dMONNIES = uint32( 0 - MONNIES );

		if( dMONNIES != 0 )
			plr->ModUnsigned32Value(PLAYER_FIELD_COINAGE, dMONNIES);
		...
		if(qst->reward_title > 0)
		{
			...
			if( !ct )
			{
				plr->GetSession()->SystemMessage("There is an error in the title-reward set for this quest(id: %u), please report on forums!", qst->id);
			}
			else if( !plr->HasTitle(ct, TITLE_TYPE_PVE) )
			{
				plr->AddTitle(ct, TITLE_TYPE_PVE );
			}
			else
			{
				plr->GetSession()->SystemMessage("You already have obtained the title that is set as a reward for this quest!");
			}
		}
...
void QuestMgr::GenerateRewardItems(Player *plr, Quest *qst)
{
	int i;
	for(i = 0; i < 4; i++)
	{
		if(qst->reward_item[i] != 0 && qst->reward_itemcount[i] > 0)
		{
			plr->GiveItem(qst->reward_item[i], qst->reward_itemcount[i]);
		}
	}
}

uint32 QuestMgr::GenerateRewardTitle(Quest * qst)
{
	uint32 rew_title = 0;
	if( qst && qst->reward_title > 0 )
	{
		CharTitlesEntry * ct = dbcCharTitle.LookupEntryForced(qst->reward_title);
		if( ct )
			rew_title = ct->bit_index;
	}
	return rew_title;
}
