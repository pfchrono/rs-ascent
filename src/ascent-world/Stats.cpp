...
uint32 CalculateXpToGive(Unit *pVictim, Unit *pAttacker)
{
	...
	if( pAttacker->IsPet() && static_cast< Pet* >( pAttacker )->GetPetOwner() )
	{
		...
		Pet *pet		= static_cast< Pet* >( pAttacker );
		uint32 ownerLvl	= pet->GetPetOwner()->getLevel();
		uint32 petLvl	= pet->getLevel();
		uint32 LevelDiff = ownerLvl - petLvl;
		VictimLvl = ( ( VictimLvl ) + ( LevelDiff * 2 ) );
		AttackerLvl = petLvl;
	}
	...
	*/
	...
	if(AttackerLvl >= 140)
		zd = 32;
	else if(AttackerLvl >= 135)
		zd = 31;
	else if(AttackerLvl >= 130)
		zd = 30;
	else if(AttackerLvl >= 125)
		zd = 29;
	else if(AttackerLvl >= 120)
		zd = 28;
	else if(AttackerLvl >= 115)
		zd = 27;
	else if(AttackerLvl >= 110)
		zd = 26;
	else if(AttackerLvl >= 105)
		zd = 25;
	else if(AttackerLvl >= 100)
		zd = 24;
	else if(AttackerLvl >= 95)
		zd = 23;
	else if(AttackerLvl >= 90)
		zd = 22;
	else if(AttackerLvl >= 85)
		zd = 21;
	...
	if( pAttacker->IsPet() )
	{
		Pet *pPet = static_cast< Pet* >( pAttacker );
		uint8 PetLoyalty = pPet->GetLoyaltyLevel();
		uint32 LoyaltyBonus = max((PetLoyalty - 2), 0);
		float RS_PETXP_Multiplier = 1.0f;
		if( VictimLvl == AttackerLvl )
		{
			RS_PETXP_Multiplier = 1.0f;
			xp = float( ( ( fVictim * 5.0f ) + ( 9.0f * ( LoyaltyBonus + 2 ) ) ) );
		}
		else if(VictimLvl > AttackerLvl)
		{
			float j = 1.0f + (0.25f * (fVictim - fAttacker) * ( LoyaltyBonus + 3 ));
			xp = float( ((AttackerLvl * 5.0f) + 10.0f) * j);
		}
		else
		{
			if((AttackerLvl - VictimLvl) < g)
			{
				...
				xp = (AttackerLvl * 5.0f + ( 9.0f * LoyaltyBonus ) * j );
			}
		}
	}
	...
	{
		if( ( ( AttackerLvl - VictimLvl ) < g ) || pAttacker->IsPet() )
		{
			float j = (1.0f - float((fAttacker - fVictim) / zd));
