...
void Apply112SpellFixes()
{
	...
	// Spell 32844 - Proc Chance (Lesser Heroism as procced by Tenacious Earthstorm Diamond)
	...
	// Spell 14177 Interrupt Flag (Cold Blood )
...
#define SET_SCHOOL(x) if( school != 0 ) { printf("Spell %u HAS MULTIPLE SCHOOLS!!!! %u\n", sp->Id, sp->School); } else { school = x; }
		...
		{
			Log.Debug("UNKNOWN SCHOOL %u for spell %u\n", sp->School, sp->Id);
		}
		*/
...
#define SET_SCHOOL(x) sp->School = x
		if(sp->School & 1)
			...
			Log.Debug("World", "UNKNOWN SCHOOL %u for spell %s (%u)\n", sp->School, sp->Name, sp->Id);
#undef SET_SCHOOL
...
//			type |= SPELL_TYPE_WARLOCK_CURSES;
		...
		else if( sp->procFlags == 0x28 )
		{
			sp->procFlags = PROC_ON_MELEE_ATTACK_VICTIM;
		}
		else if( sp->procFlags == 0x2A8 && sp->Attributes&0x50000 && sp->AttributesExC&0x2 )
		{
			sp->procFlags = PROC_ON_BLOCK_VICTIM;
		}
		else if( sp->procFlags == 0x2A8 )
		{
			sp->procFlags = PROC_ON_MELEE_ATTACK_VICTIM | PROC_ON_RANGED_ATTACK_VICTIM;
		}
		else if( sp->procFlags == 0x10000 )
		{
			sp->procFlags = PROC_ON_CAST_SPELL;
		}
		else if( sp->procFlags == 0x15550 && ( sp->AttributesExD&0x40 || sp->AttributesEx&0x20 ) )
		{
			sp->AuraInterruptFlags = AURA_INTERRUPT_ON_CAST_SPELL;
		}
		else if( sp->procFlags == 0x15550 )
		{
			sp->procFlags = PROC_ON_CAST_SPELL;
		}
		else if (sp->procFlags == 0x222A8 && sp->Attributes == 0x1D0 && !(sp->AttributesExD&0x80000))
		{
			sp->procFlags = PROC_ON_CRIT_HIT_VICTIM | PROC_ON_RANGED_CRIT_ATTACK_VICTIM | PROC_ON_SPELL_CRIT_HIT_VICTIM;
		}
		else if (sp->procFlags == 0x222A8)
		{
			sp->procFlags = PROC_ON_SPELL_HIT_VICTIM | PROC_ON_RANGED_ATTACK_VICTIM | PROC_ON_MELEE_ATTACK_VICTIM;
		}
		else if (sp->procFlags == 0xA22A8)
		{
			// ON_HEALTH_DROP
		}
		else if (sp->procFlags == 0x14 && sp->AttributesExC&0x4000000)
		{
			sp->procFlags = PROC_ON_CRIT_ATTACK;
		}
		else if (sp->procFlags == 0x14)
		{
			sp->procFlags = PROC_ON_MELEE_ATTACK;
		}
		else if (sp->procFlags == 0x1)
		{
		}
		else if (sp->procFlags == 0x2)
		{
		}
		else if (sp->procFlags == 0x100000)
		{
		}
		else if (sp->procFlags == 0x11154 && sp->Attributes == 0x1D0)
		{
			sp->procFlags = PROC_ON_CRIT_ATTACK | PROC_ON_RANGED_CRIT_ATTACK | PROC_ON_SPELL_CRIT_HIT;
		}
		else if (sp->procFlags == 0x11154 && sp->Attributes == 0x50010)
		{
			sp->procFlags = PROC_ON_MELEE_ATTACK;
		}
		else if (sp->procFlags != 0)
		{
...
/*		//if there is a proc spell and has 0 as charges then it's probably going to triger infinite times. Better not save these
		...
		/* hackfix for this - FIX ME LATER - Goatlex */
...
*/
		...
		if(sp->NameHash == SPELL_HASH_WINDFURY_WEAPON ||
			...
			sp->AttributesExB |= FLAGS3_ENCHANT_OWN_ONLY;
		...
		if( sp->NameHash == SPELL_HASH_DIVINE_ILLUMINATION )
			sp->procFlags |= PROC_TARGET_SELF;
...
*/
	...
	if( Enchantment != NULL )
	{
		...
		if( sp != NULL )
		{
			...
			sp->EffectTriggerSpell[1] = 33010; //animation
			sp->procChance = 20;
...
//		sp->Effect[0] = SPELL_EFFECT_POWER_BURN; // should be Power Burn, not Power Drain. Power Drain leeches mana which is incorrect.
	...
	// Idle
	sp = dbcSpell.LookupEntryForced( 43680 );
	if( sp != NULL )
	{
		sp->AuraInterruptFlags = AURA_INTERRUPT_ON_CAST_SPELL | AURA_INTERRUPT_ON_START_ATTACK;
	}
	// Inactive
	sp = dbcSpell.LookupEntryForced( 43681 );
	if( sp != NULL )
	{
		sp->AuraInterruptFlags = AURA_INTERRUPT_ON_CAST_SPELL | AURA_INTERRUPT_ON_START_ATTACK;
	}
	...
	//Shadowmeld
	sp = dbcSpell.LookupEntryForced(20580);
	if( sp != NULL )
		sp->AuraInterruptFlags |= AURA_INTERRUPT_ON_CAST_SPELL;
	...
	//Lesser Invisibility
	sp = dbcSpell.LookupEntryForced(3680);
	if( sp != NULL )
		sp->AuraInterruptFlags |= AURA_INTERRUPT_ON_CAST_SPELL;
	...
	// Eye for an Eye - Not affected by Bonus Damage
	sp = dbcSpell.LookupEntryForced( 9799 );
	if( sp != NULL )
	{
		sp->fixed_dddhcoef = 0.0f; // disable DD +spell damage
	}
	...
	**********************************************************/
	...
	if( sp != NULL )
	{
		sp->procFlags = PROC_ON_TRAP_TRIGGER;			//sp->EffectTriggerSpell[0] = 13810;
...
*/
	...
	// Swift Magic Broom
	sp = dbcSpell.LookupEntryForced( 42683 );
