...
World::~World()
{
	...
	Log.Notice("BugReporter", "~BugReporter()");
	delete BugReporter::getSingletonPtr();
...
#define MAKE_TASK(sp, ptr) tl.AddTask(new Task(new CallbackP0<sp>(sp::getSingletonPtr(), &sp::ptr)))
	...
	MAKE_TASK(BugReporter, LoadBugReports);
	...
	MAKE_TASK(ObjectMgr, LoadSpellSpellRelation);
...
void World::SendGamemasterMessage(WorldPacket *packet)
{
	...
	for(itr = m_sessions.begin(); itr != m_sessions.end(); itr++)
	{
		if (itr->second->GetPlayer() 
			&& itr->second->GetPlayer()->IsInWorld()
			&& itr->second->GetPlayer()->RS_getAccess() >= RS_PROBEGM
			)
			itr->second->SendPacket(packet);
	}
	m_sessionlock.ReleaseReadLock();
...
void World::SendGMWorldText(const char* text)
{
	uint32 textLen = (uint32)strlen((char*)text) + 1;
...
void World::SaveAllPlayers()
{
	...
	for (itr = objmgr._players.begin(); itr != objmgr._players.end(); itr++)
	{
		...
		{
			mt = getMSTime();
			itr->second->SaveToDB(false, true);
...
void World::Rehash(bool load)
{
	if(load)
	{
		...
		Config.RS_Config.SetSource("ascent-rockstar.conf", true);
		Config.AH_Config.SetSource("ascent-ahbot.conf", true);
		#else
		Config.MainConfig.SetSource((char*)CONFDIR "/ascent-world.conf", true);
		Config.RS_Config.SetSource((char*)CONFDIR "/ascent-rockstar.conf", true);
		Config.AH_Config.SetSource((char*)CONFDIR "/ascent-ahbot.conf", true);
		#endif
	}
	...
	log_enabled = Config.RS_Config.GetBoolDefault("Log", "Chat", false);
	if(Chat_Log->IsOpen() && !log_enabled)
		Chat_Log->Close();
	else if(log_enabled)
		Chat_Log->Open();

	log_enabled = Config.RS_Config.GetBoolDefault("Log", "Channels", false);
	if(Chan_Log->IsOpen() && !log_enabled)
		Chan_Log->Close();
	else if(log_enabled)
		Chan_Log->Open();

	log_enabled = Config.RS_Config.GetBoolDefault("Log", "Sessions", false);
	if(Session_Log->IsOpen() && !log_enabled)
		Session_Log->Close();
	else if(log_enabled)
		Session_Log->Open();

	log_enabled = Config.RS_Config.GetBoolDefault("Log", "MailInsert", false);
	if(MailInsert_Log->IsOpen() && !log_enabled)
		MailInsert_Log->Close();
	else if(log_enabled)
		MailInsert_Log->Open();

	log_enabled = Config.RS_Config.GetBoolDefault("Log", "Sent_Mail", false);
	if(SentMail_Log->IsOpen() && !log_enabled)
		SentMail_Log->Close();
	else if(log_enabled)
		SentMail_Log->Open();
	...
	antiMasterLootNinja = Config.RS_Config.GetBoolDefault("Server", "AntiMasterLootNinja", false);
	map_unload_time=Config.MainConfig.GetIntDefault("Server", "MapUnloadTime", 0);

	bFallDamage = Config.RS_Config.GetBoolDefault("Server", "FallDamage", false);
	...
	// ======================================
	rsWarnSystem	= Config.RS_Config.GetBoolDefault("Commands", "WarnSystem", true);
	rsReportPlr		= Config.RS_Config.GetBoolDefault("Commands", "ReportPlr", true);
	rsChatTag		= Config.RS_Config.GetBoolDefault("Commands", "ChatTag", true);
	rsBugReportSys	= Config.RS_Config.GetBoolDefault("Commands", "BugReportSystem", true);
	rsChanInCon		= Config.RS_Config.GetBoolDefault("Console",  "ChannelInConsole", true);
	rsWhispInCon	= Config.RS_Config.GetBoolDefault("Console",  "WhisperInConsole", false);
	// ======================================
	logon_pass		= Config.MainConfig.GetStringDefault("LogonServer", "RemotePassword", "r3m0t3b4d");
	// ======================================

	#ifdef COLLISION
		Log.Notice("Terrain", "Collision is enabled!");
	#else
		Log.Notice("Terrain", "Collision is disabled!");
	#endif
...
bool CharacterLoaderThread::run()
{
	...
	for(;;)
	{
		...
		sWorld.ToolDisconnectPlayer(con);
...
void World::ToolDisconnectPlayer(DatabaseConnection * con)
{
	QueryResult * result = CharacterDatabase.FQuery("SELECT * FROM disconnectionqueue", con);
	if( result != NULL )
	{
		Log.Success("DisconnectQueue", "Disconnecting queued characters....");
		do
		{
			Player * _plr			= NULL;
			string			cStr	= result->Fetch()[0].GetString();
			const char*		plrName = WorldDatabase.EscapeString( cStr.c_str() ).c_str();
			PlayerInfo * _plrI		= objmgr.GetPlayerInfoByName(plrName);
			if(_plrI)
			{
				_plr = objmgr.GetPlayer(_plrI->guid);
			}
			if( _plr != NULL )
			{
				if( _plr->IsInWorld() )
				{
					if( _plr->GetSession() )
						...
						_plr->RemoveFromWorld();
				}
			}
		} while( result->NextRow() );
		delete result;
		CharacterDatabase.FWaitExecute("DELETE FROM disconnectionqueue", con);
	}
}
...
void World::PollMailboxInsertQueue(DatabaseConnection * con)
{
	...
	if( result != NULL )
	{
		...
		do 
		{
			...
			playerguid = f[1].GetUInt64();
			VoteLogEntry = f[8].GetUInt32();
			if( itemid != 0 )
			{
			...
			if(VoteLogEntry > 0)
				WorldDatabase.Execute("UPDATE `vote_log` SET `delivered` = '1' WHERE `VoteLogEntry` = '%u'", VoteLogEntry);

			Log.Notice("MailboxQueue", "Sending message to %u (item: %u)...", f[1].GetUInt32(), itemid);
			sMailSystem.SendAutomatedMessage( 0, f[0].GetUInt64(), playerguid, f[2].GetString(), f[3].GetString(), f[5].GetUInt32(),
				0, pItem ? pItem->GetGUID() : 0, f[4].GetUInt32() );

			pInfo = objmgr.GetPlayerInfo( GUID_LOPART(playerguid) );
			if(pInfo)
				sMailLog.write("external mail message sent to player %s (guid: %u - acct: %u) - Attached Item: %u", pInfo->name, pInfo->guid, pInfo->acct, pItem?pItem->GetEntry():0);
...
void World::LogWriteFromSession(WorldSession * m_session, uint32 LogType, const char * text)
{
	if(!m_session || strlen(text) < 1)
		return;
	
	switch(LogType)
	{
	case CHEATLOG:
		sCheatLog.writefromsession(m_session, text);
		break;
	case GMLOG:
		sGMLog.writefromsession(m_session, text);
		break;
	case CHATLOG:
		sChatLog.writefromsession(m_session, text);
		break;
	case CHANLOG:
		sChanLog.writefromsession(m_session, text);
		break;
	}
}

void World::CallAccountReload()
{
...
class NewsAnnouncer : public ThreadBase
{
	bool running;
#ifdef WIN32
	HANDLE hEvent;
...
void World::StartAnnouncer()
{
	// Try to connect to all logons.
	Log.Notice("NewsAnnouncer", "Loading announcement strings...");
