...
uint32 InstanceMgr::PreTeleport(uint32 mapid, Player * plr, uint32 instanceid)
{
	...
	if( (pGroup == NULL) && ( (inf->type == INSTANCE_MULTIMODE) && (plr->iInstanceType < MODE_HEROIC)) && !plr->triggerpass_cheat )
		return INSTANCE_ABORT_NOT_IN_RAID_GROUP;

	// check that heroic mode is available if the player has requested it.
	if( plr->iInstanceType >= MODE_HEROIC && inf->type != INSTANCE_MULTIMODE )
...
MapMgr * InstanceMgr::_CreateInstance(uint32 mapid, uint32 instanceid)
{
	...
	Log.Debug("InstanceMgr", "Creating continent %u (%s).", mapid, m_maps[mapid]->GetName());
...
{
	Log.Debug("InstanceMgr", "Creating saved instance %u (%s)", in->m_instanceId, m_maps[in->m_mapId]->GetName());
...
void InstanceMgr::_LoadInstances()
{
	...
	Log.Debug("InstanceMgr", "Deleting Expired Instances...");
	...
	Log.Debug("InstanceMgr", "Loading %u saved instances." , result ? result->GetRowCount() : 0);
