...
{	//											Hex			Bit		Decimal		Comments
	...
	UNIT_FLAG_PREPARATION					= 0x00000020, // 6		32
...
public:
	...
	ASCENT_INLINE bool IsInCombat();												// checks if we are in combat or not.
...
//	void HandleProcSpellOnSpell(Unit* Victim,uint32 damage,bool critical);//nasty, some spells proc other spells
	...
	Unit* create_guardian( uint32 guardian_entry, uint32 duration, float angle, uint32 lvl = 0, GameObject * obj = NULL, Spell * spell_callback = NULL );//guardians are temporary spawn that will inherit master faction and will folow them. Apart from that they have their own mind
...
//	int32 * SM_PAmptitude;
	...
	Unit* GetCharmerOrOwner() { return GetCharmerGUID() ? GetCharmer() : GetOwner(); }
	...
	uint64 GetOwnerGUID() { return  GetUInt64Value(UNIT_FIELD_SUMMONEDBY); }
	uint64 GetPetGUID() { return  GetUInt64Value(UNIT_FIELD_SUMMON); }
	uint64 GetCharmerGUID() { return GetUInt64Value(UNIT_FIELD_CHARMEDBY); }
	uint64 GetCharmGUID() { return  GetUInt64Value(UNIT_FIELD_CHARM); }
	void SetCharmerGUID(uint64 owner) { SetUInt64Value(UNIT_FIELD_CHARMEDBY, owner); }

	uint64 GetCharmerOrOwnerGUID() { return ( GetCharmerGUID() ? GetCharmerGUID() : GetOwnerGUID() ); }
	uint64 GetCharmerOrOwnerOrOwnGUID()
	{
	...
	bool isCharmedOwnedByPlayerOrPlayer() { return IS_PLAYER_GUID(GetCharmerOrOwnerOrOwnGUID()); }
	...
	ASCENT_INLINE float GetSpeed(uint8 type = 1)
	{
		switch(type)
		{
		case 1:
...
//	bool m_spellsbusy;
	...
	Object * pLastAttacker; // PyRo - 1/10/08
