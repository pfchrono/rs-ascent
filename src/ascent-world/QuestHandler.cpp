...
void WorldSession::HandleQuestGiverQueryQuestOpcode( WorldPacket & recv_data )
{
	...
	{
		if( qst->is_repeatable == QUEST_REPEATABLE_DAILY && ( _player->ActiveDailiesCount() + _player->GetFinishedDailyCount() >= 25 ) )
		{	
			sQuestMgr.SendQuestInvalid(INVALID_REASON_REACHED_DAILY_LIMIT, _player);
			_player->Gossip_Complete();
			return;
		}
