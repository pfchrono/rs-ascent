...
void WorldSession::HandleSwapItemOpcode(WorldPacket& recv_data)
{
	...
	if( SrcSlot < EQUIPMENT_SLOT_END || DstSlot < EQUIPMENT_SLOT_END )			// We're doing an equip swap.
	{
		if( _player->isDead() )
		{
			_player->GetItemInterface()->BuildInventoryChangeError(SrcItem, DstItem, INV_ERR_YOU_ARE_DEAD);
			return;
		}
	}

	if( !_player->GetItemInterface()->CanSwapSimplicityItem( SrcItem, ( DstItem ? DstItem : NULL ) ) )
	{
		_player->GetItemInterface()->BuildInventoryChangeError(SrcItem, DstItem ? DstItem : NULL, INV_ERR_CANT_DO_RIGHT_NOW);
		return;
	}
...
/*	if( !iInt->CanSwapSimplicityItem( srcitem, ( dstitem ? dstitem : NULL ) ) )
	{
		iInt->BuildInventoryChangeError(srcitem, dstitem ? dstitem : NULL, INV_ERR_CANT_DO_RIGHT_NOW);
		return;
	}*/
	...
	if( ( error = _player->GetItemInterface()->CanEquipItemInSlot2( INVENTORY_SLOT_NOT_SET, dstslot, srcitem, skip_combat ) ) )
	{
		if( dstslot < INVENTORY_KEYRING_END )
		{
			if ( error == INV_ERR_CAN_EQUIP_ONLY1_AMMOPOUCH )
			{
				// trap error so we can swap ammo bags in the bag bar properly
				if ( !_player->GetItemInterface()->IsBagSlot(srcslot) || !_player->GetItemInterface()->IsBagSlot(dstslot) )
				{
					// at least one slot not from bag bar
					if ( !dstitem || ( dstitem->GetProto()->Class != ITEM_CLASS_QUIVER ) )
					{
						// not swapping quiver in bag bar, proceed with error
						_player->GetItemInterface()->BuildInventoryChangeError( srcitem, dstitem, error );
						return;
					}
				}
	...
	// exra check needed here for gems, because CanEquipInSlot does not pass enchantments, etc.
	...
	if( dstslot < EQUIPMENT_SLOT_END && srcitem->HasEnchantments() )
	{
		...
		{
			if( srcitem->GetEnchantment( x ) && srcitem->GetEnchantment( x )->Enchantment )
			{
				gemitm = ItemPrototypeStorage.LookupEntry( srcitem->GetEnchantment( x )->Enchantment->GemEntry );
				if( !gemitm )
					continue; // baaad boy
				if( ( gemitm->Flags & ITEM_FLAG_UNIQUE_EQUIP ) && _player->GetItemInterface()->HasGemEquipped( srcitem->GetEnchantment( x )->Enchantment->GemEntry , dstslot ) )
				{
	...
	if(dstitem)
	{
		if((error=_player->GetItemInterface()->CanEquipItemInSlot2(INVENTORY_SLOT_NOT_SET, srcslot, dstitem, skip_combat)) )
		{
			if ( error == INV_ERR_CAN_EQUIP_ONLY1_AMMOPOUCH )
			{
				...
				{
					 // at least one slot not from bag bar
					if ( srcitem->GetProto()->Class != ITEM_CLASS_QUIVER )
					{
						// not swapping quiver in bag bar, procede with error
						_player->GetItemInterface()->BuildInventoryChangeError( dstitem, srcitem, error );
						return;
					}
				}
			}
	...
	if(srcitem->IsContainer())
	{
		...
		if(((Container*)srcitem)->HasItems() && !_player->GetItemInterface()->IsBagSlot(dstslot))
		{
			_player->GetItemInterface()->BuildInventoryChangeError(srcitem,dstitem, INV_ERR_NONEMPTY_BAG_OVER_OTHER_BAG);
			return;
		}
...
/*	if( !_player->GetItemInterface()->CanSwapSimplicityItem( eitem, (_player->GetItemInterface()->GetInventoryItem( Slot )) ) )
	{
		_player->GetItemInterface()->BuildInventoryChangeError(eitem, dstitem ? dstitem : NULL, INV_ERR_CANT_DO_RIGHT_NOW);
		return;
	}
*/
	Item * oitem = NULL;
...
/*	if(amount == 0)
	...
	uint64 ownerguid = item->GetUInt32Value(ITEM_FIELD_OWNER);
	if( ownerguid == NULL || ownerguid != _player->GetGUID() )
	{
		item->RemoveFromWorld();
		item->DeleteFromDB();
		sCheatLog.writefromsession(_player->GetSession(), "tried to sell an item that had owner==NULL, probable duper!");
		SendSellItem(vendorguid, itemguid, 1);
		return; //our player doesn't have this item
	}
	...
	uint32 sellprice = GetSellPriceForItem(it, quantity);

	uint32 buyprice = GetBuyPriceForItem(it, quantity, _player, unit);
	if( sellprice > buyprice )
	{
		sCheatLog.writefromsession(_player->GetSession(), "sold item %u for %u copper, while buyprice is lower ( %u )", it->ItemId, sellprice, buyprice);
	}
	if(_player->GetUInt32Value(PLAYER_FIELD_COINAGE) + sellprice > GOLDLIMIT)
	{
//		SendSellItem(vendorguid, itemguid, 2);
		_player->GetItemInterface()->BuildInventoryChangeError(item, NULL, INV_ERR_TOO_MUCH_GOLD);
		return;
	}
	_player->ModUnsigned32Value(PLAYER_FIELD_COINAGE, sellprice);
...
void WorldSession::SendInventoryList(Creature* unit)
{
	...
	for(std::vector<CreatureItem>::iterator itr = unit->GetSellItemBegin(); itr != unit->GetSellItemEnd(); ++itr)
	{
		if(itr->itemid && (itr->max_amount == 0 || (itr->max_amount>0 && itr->available_amount >0)))
		{
			...
			{
				if(curItem->AllowableClass && !(_player->getClassMask() & curItem->AllowableClass) && _player->RS_getAccess() < RS_PROBEGM ) // GMs can see all items
					continue;
				if(curItem->AllowableRace && !(_player->getRaceMask() & curItem->AllowableRace) && _player->RS_getAccess() < RS_PROBEGM) // GMs can see all items
					continue;
...
{
	if( pItem->GetProto()->Unrepairable == ITEM_UNREPAIRABLE )
		return 0;
	...
	if( pItem->GetProto()->Quality == ITEM_QUALITY_ARTIFACT_LIGHT_YELLOW )
	{
		dquality = dbcDurabilityQuality.LookupEntry((ITEM_QUALITY_LEGENDARY_ORANGE + 1) * 2);
	}
	if(!dquality)
	{
...
ASCENT_INLINE bool RepairItem(Player * pPlayer, Item * pItem)
{
...
void WorldSession::HandleRepairItemOpcode(WorldPacket &recvPacket)
{
	...
	if( !itemguid ) 
	{
		for( i = 0; i < MAX_INVENTORY_SLOT; i++ )
		{
			...
			if( pItem != NULL && pItem->GetProto()->Unrepairable == ITEM_REPAIRABLE )
			{
	...
	else 
	{
		...
		if(item)
		{
			if( item->GetProto()->Unrepairable == ITEM_UNREPAIRABLE )
			{
				_player->SendAreaTriggerMessage("You can not repair this item!");
				return;
			}
