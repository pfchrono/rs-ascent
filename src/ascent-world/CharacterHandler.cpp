...
void WorldSession::HandleCharCreateOpcode( WorldPacket & recv_data )
{
	...
	if( !HasGMPermissions() && realmType == REALMTYPE_PVP && _side >= 0 )
	{
...
void WorldSession::FullLogin(Player * plr)
{
	...
	if (mgr && mgr->m_battleground)
	{
		...
		if (mgr->m_battleground->HasEnded() == true ||
			mgr->m_battleground->HasFreeSlots(plr->GetTeamInitial()) == false ) { //, mgr->m_battleground->GetType() == false)) {
				mgr = NULL;
		}
	}
...
//	-- Dealt with in player->SendDungeonDifficulty() at the end of this function.
	...
	*/
	...
	if(info->guild)
	{
		plr->m_uint32Values[PLAYER_GUILDID] = info->guild->GetGuildId();
		plr->m_uint32Values[PLAYER_GUILDRANK] = info->guildRank->iId;
	}

	/*ArenaTeam * aTeam = NULL;
	for(uint32 z = 0; z < NUM_ARENA_TEAM_TYPES; ++z)
	{
		aTeam = info->arenaTeam[z];
		if(aTeam != NULL)
		{
			plr->m_uint32Values[PLAYER_FIELD_ARENA_TEAM_INFO_1_1 + (z*6)] = aTeam->m_id;
			if(aTeam->m_leader == plr->GetLowGUID())
				plr->m_uint32Values[PLAYER_FIELD_ARENA_TEAM_INFO_1_1 + (z*6) + 1] = 0;
			else
				plr->m_uint32Values[PLAYER_FIELD_ARENA_TEAM_INFO_1_1 + (z*6) + 1] = 1;
		}
	}*/
...
#ifdef WIN32
	_player->BroadcastMessage("Server: %sRockStar(%sAscent Core%s) %s r%u/%s-Win-%s %s(www.fishstik.ca)", MSG_COLOR_WHITE, MSG_COLOR_LIGHTBLUE, 
		MSG_COLOR_WHITE, BUILD_TAG, BUILD_REVISION, CONFIG, ARCH, MSG_COLOR_LIGHTBLUE);
#else
	_player->BroadcastMessage("Server: %sRockStar(%sAscent Core%s) %s r%u/%s-%s %s(www.fishstik.ca)", MSG_COLOR_WHITE, BUILD_TAG,
		MSG_COLOR_LIGHTBLUE, MSG_COLOR_WHITE, BUILD_REVISION, PLATFORM_TEXT, ARCH, MSG_COLOR_LIGHTBLUE);
#endif

	_player->BroadcastMessage("%sDon't forget to /join world and /join gmhelp %sAnd %sdon't forget to read the rules at www.fishstik.ca!", 
		MSG_COLOR_LIGHTBLUE, MSG_COLOR_WHITE, MSG_COLOR_LIGHTBLUE);
