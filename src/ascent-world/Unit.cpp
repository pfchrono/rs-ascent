...
#include "StdAfx.h"
#pragma warning(disable:4018 244 293)
...
uint32 Unit::HandleProc( uint32 flag, Unit* victim, SpellEntry* CastingSpell, uint32 dmg, uint32 abs , uint32 weapon_damage_type)
{
	...
	for( itr = m_procSpells.begin(); itr != m_procSpells.end(); )  // Proc Trigger Spells for Victim
	{
		...
		if( itr2 == NULL && itr2 > m_procSpells.begin() && itr2 < m_procSpells.end()n)
		{		
						...
						case 25505:
							{
								...
								Strike( victim, weapon_damage_type-1, spe, extra_dmg, 0, 0, true, false );
								Strike( victim, weapon_damage_type-1, spe, extra_dmg, 0, 0, true, false );
...
//==========================================================================================
	...
	if (IsPlayer())
	{
		...
		{
			if(static_cast<Player*>(this)->DuelingWith != pVictim && pVictim->IsPlayer() == true)
			{
...
//--------------------------------miss------------------------------------------------------
	case 0:
		...
		// dirty ai agro fix
		if(pVictim->GetTypeId() == TYPEID_UNIT && pVictim->isAlive() && pVictim->GetAIInterface()->GetNextTarget() == NULL)
...
//--------------------------special states processing---------------------------------------
	if(pVictim->GetTypeId() == TYPEID_UNIT && pVictim->isAlive())
	{
...
//--------------------------dirty fixes-----------------------------------------------------
	...
	if( !disable_proc && pVictim->isAlive() ) // && weapon_damage_type != OFFHAND  )
	{
		uint32 resisted_dmg;
		...
		if( ability && ability->Id )
		{
			...
			resisted_dmg = pVictim->HandleProc( vproc, this, ability, realdamage, abs, weapon_damage_type + 1);
...
//--------------------------spells triggering-----------------------------------------------
	...
	{
		if( IsPlayer() && static_cast< Player* >( this )->m_onStrikeSpells.size() && pVictim->isAlive() )
		{
			SpellCastTargets targets;
		...
		if( IsPlayer() && static_cast< Player* >( this )->m_onStrikeSpellDmg.size() && pVictim->isAlive() )
		{
...
//--------------------------durability processing-------------------------------------------
	...
	if( dmg.full_damage && pVictim->IsPlayer() && pVictim->GetPowerType() == POWER_TYPE_RAGE && pVictim->CombatStatus.IsInCombat() && pVictim->isAlive() )
	{
		float val;
...
void Unit::AddAura(Aura *aur)
{
	...
	if( !aur->IsPassive() && aur->GetSpellProto()->always_apply == false )
	{
...
void Unit::DeMorph()
{
	...
	this->SetFloatValue(OBJECT_FIELD_SCALE_X, 1.0f);
	EventModelChange();
}
...
void Unit::DropAurasOnDeath()
{
	...
	for(uint32 x = 0; x < MAX_AURAS + MAX_PASSIVE_AURAS; x++)
	{
		...
		if(aur != NULL)
		{
			SE = aur->GetSpellProto();
			if(SE != NULL)
			{
				if( !HasMask(SE->AttributesExC, CAN_PERSIST_AND_CASTED_WHILE_DEAD) )
				{
					aur->Remove();
				}
				if( SE != NULL)
...
AuraCheckResponse Unit::AuraCheck(uint32 name_hash, uint32 rank, Aura* aur, Object *caster)
{
	...
	if( aur == NULL || aur->GetSpellProto() == NULL )
		return resp;
...
Unit* Unit::create_guardian( uint32 guardian_entry, uint32 duration, float angle, uint32 lvl, GameObject * obj, Spell * spell_callback )
{
	...
	{
		SpellCastTargets sct = spell_callback->m_targets;
		x += sct.m_destX;
		y += sct.m_destY;
		z += sct.m_destZ;
	}
...
void CombatStatusHandler::OnDamageDealt(Unit * pTarget)
{
	...
	if( m_Unit->IsPlayer() && !pTarget->IsPlayer() )
	{
...
{
	if( !m_Unit || !m_Unit->GetMapMgr() || !m_Unit->IsInWorld() )
...
ASCENT_INLINE bool CombatStatusHandler::IsInCombat()
{
	if (!m_Unit || !m_Unit->IsInWorld() || !m_Unit->GetAIInterface() || !m_lastStatus)
		return false;
...
Unit *Unit::GetOwner()
{
	...
	if(!ownerid || !GetMapMgr())
		return NULL;

	return GetMapMgr()->GetUnit( ownerid );
}

Unit *Unit::GetCharmer()
{
	uint64 charmerid = GetCharmerGUID();
	if(!charmerid || !GetMapMgr())
		return NULL;
	
	return GetMapMgr()->GetUnit( charmerid );
}
...
Player* Unit::GetCharmerOrOwnerPlayerOrPlayerItself()
{
	...
	if(IS_PLAYER_GUID(guid))
		return objmgr.GetPlayer( GUID_LOPART(guid) );
