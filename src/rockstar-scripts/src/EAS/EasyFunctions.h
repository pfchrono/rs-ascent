 ...
 * RockStar Ascent Modifications by Midnight, Egari, Toast
 * Copyright (C) 2005-2007 Ascent Team <http://www.ascentemu.com/>
 * Copyright (C) 2007-2008 Moon++ Team <http://www.moonplusplus.info/>
 * Copyright (C) 2007-2008 RockStar Wow Team <http://www.fishstik.ca/>
 *
...
#define CREATE_DUMMY_SPELL(cl) &cl

/* New Macro's to make my life easier and lazier - Midnight */
#define CQS(cl) CREATE_QUESTSCRIPT(cl)
#define CGS(cl) CREATE_GOSSIPSCRIPT(cl)
#define CGOS(cl) CREATE_GAMEOBJECT_SCRIPT(cl)
#define CCS(cl) CREATE_CREATURESCRIPT(cl)
#define CDS(cl) CREATE_DUMMY_SPELL(cl)
...
Creature *SpawnCreature(Player *pThis, uint32 entry, float posX, float posY, float posZ, float posO, uint32 duration = 0)
{
	...
	pCreature->spawnid = 0;
	pCreature->m_spawn = 0;
	pCreature->Load(p, posX, posY, posZ, posO);
...
bool InGroupHasQuest(Player * plr, uint32 QuestEntry)
{
	Player *gplr = NULL;
	if(plr->InGroup())
	{
		if(Group* pGroup = plr->GetGroup())
		{
			...
			for(uint32 k = 0; k < pGroup->GetSubGroupCount(); k++)
			{
				for(gitr = pGroup->GetSubGroup(k)->GetGroupMembersBegin(); gitr != pGroup->GetSubGroup(k)->GetGroupMembersEnd(); ++gitr)
				{
					...
					if(gplr)
					{
						QuestLogEntry	*en = plr->GetQuestLogForEntry(QuestEntry);
						/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
						if(en)
						{
							return true;
