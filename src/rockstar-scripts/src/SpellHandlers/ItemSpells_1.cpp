/*
	 SpellHandler Plugin Copyright (c) 2007 Team Ascent This work is licensed under
	 the Creative Commons Attribution-NonCommercial-ShareAlike 3.0 License. To view
	 ...
	 Francisco, California, 94105, USA. EXCEPT TO THE EXTENT REQUIRED BY APPLICABLE
...
bool NoggenFoggerElixr(uint32 i, Spell *pSpell)
{
	...
	switch(chance)
	{
	case 0: pSpell->p_caster->CastSpell(pSpell->p_caster, dbcSpell.LookupEntry(16591), true); break;
	case 1: pSpell->p_caster->CastSpell(pSpell->p_caster, dbcSpell.LookupEntry(16593), true); break;
	case 2: pSpell->p_caster->CastSpell(pSpell->p_caster, dbcSpell.LookupEntry(16595), true); break;
	}
...
bool CookedDeviateFish(uint32 i, Spell *pSpell)
{
	...
	switch(chance)
	{
	case 0: newspell = 8219;					/* Flip Out (60 min) (turns you into a ninja) */break;
	case 1: newspell = 8221;					/* Yaaarrrr (60 min) (turns you into a pirate) */break;
	}
...
bool HolidayCheer(uint32 i, Spell *pSpell)
{
	...
	for
	(
		ObjectSet::iterator itr = pSpell->m_caster->GetInRangeSetBegin();
		itr != pSpell->m_caster->GetInRangeSetEnd();
		++itr
	)
	{
		if((*itr)->IsUnit())
		...
		if(pSpell->m_caster->CalcDistance(target) > dist || isAttackable(pSpell->m_caster, target)) continue;
...
bool BanishExile(uint32 i, Spell *pSpell)
{
	...
	pSpell->p_caster->SpellNonMeleeDamageLog
		(
			target,
			pSpell->GetProto()->Id,
			target->GetUInt32Value(UNIT_FIELD_HEALTH),
			true
		);
	return true;
}
...
bool ForemansBlackjack(uint32 i, Spell *pSpell)
{
	...
	if(creatureID == 10556) sQuestMgr.OnPlayerKill(pSpell->p_caster, c_target);
...
{
	/*
		 // commented out until the SpellTargetDummy (38) targeting is fixed Unit*
		 target = pSpell->GetUnitTarget();
		 if(!pSpell->p_caster || !target || target->GetTypeId() != TYPEID_UNIT) return true;
		 Creature* c_target = (Creature*)target;
		 if(!c_target) return true;
		 uint32 creatureID = c_target->GetEntry();
...
bool SummonCritterDummy(uint32 i, Spell *pSpell)
{
	...
	pSpell->p_caster->CastSpell(pSpell->p_caster, spInfo, false);	/* these spells have to check items, so
																	 * "triggeredspell" must be false */
	return true;
}
// *****************************************************************************
...
// *****************************************************************************
bool PiccoloOfFlame(uint32 i, Spell *pSpell)
{
	Player * Plr = pSpell->p_caster;
	for(set<Player*>::iterator itr = Plr->GetInRangePlayerSetBegin(); itr != Plr->GetInRangePlayerSetEnd(); ++itr)
	{
		if((*itr)->IsPlayer() && (*itr)->isAlive() && (*itr)->GetSession() && !(*itr)->IsAttacking())
		{
			(*itr)->CastSpell((*itr), 18400, true);
		}
	}
	return true;
}
...
bool HallowedWandNinja(uint32 i, Spell *pSpell)
{
	...
	{
		pSpell->p_caster->CastSpell(target, dbcSpell.LookupEntry(24710), true);	//female
	}
	else
	{
		pSpell->p_caster->CastSpell(target, dbcSpell.LookupEntry(24711), true);	 //male
	}
	return true;
}
// *****************************************************************************
...
// *****************************************************************************
bool HallowedWandRandom(uint32 i, Spell *pSpell)
{
	if(!pSpell->p_caster) return true;
	Unit *target = pSpell->GetUnitTarget();
	...
	if(target->getGender() == 1)
	{
		const uint32 spellList[7] = { 24709, 24710, 24713, 24723, 24732, 24736, 24740 };
		sp = dbcSpell.LookupEntryForced( spellList[ chance ] );
	}
	else
	{
		const uint32 spellList[7] = { 24708, 24711, 24712, 24723, 24732, 24735, 24740 };
		sp = dbcSpell.LookupEntryForced( spellList[ chance ] );
	}

	if( sp != NULL )
		pSpell->p_caster->CastSpell(target, sp, true);
...
bool BagOfCandies(uint32 i, Spell *pSpell)
{
	...
	uint32 item = RandomUInt(6);

	sEAS.AddItem(21817 + item, plr);
...
bool DarkmoonCard_Madness(uint32 i, Spell *pSpell)
{
	...
	uint32 chance = 0;
	SpellEntry * spellProto = NULL;

	if( caster->getClass() == PALADIN || caster->getClass() == DRUID )
	{
		uint32 chance = RandomUInt(8);
		const uint32 spellList[9] = {39511, 40997, 40998, 40999, 41002, 41005, 41009, 41011, 41409};
		spellProto = dbcSpell.LookupEntry( spellList[chance] );
	}

	if( caster->getClass() == ROGUE || caster->getClass() == WARRIOR || caster->getClass() == PRIEST || caster->getClass() == SHAMAN )
	{
		uint32 chance = RandomUInt(5);
		const uint32 spellList[6] = {39511, 40997, 40998, 41002, 41005, 41011};
		spellProto = dbcSpell.LookupEntry( spellList[chance] );
	}
	...
	{
		uint32 chance = RandomUInt(6);
		const uint32 spellList[7] = {40999, 41002, 41005, 41009, 41011, 41406, 41409};
		spellProto = dbcSpell.LookupEntry( spellList[chance] );
	}
	...
	{
		uint32 chance = RandomUInt(7);
		const uint32 spellList[8] = {40997, 40999, 41002, 41005, 41009, 41011, 41406, 41409};
		spellProto = dbcSpell.LookupEntry( spellList[chance] );
	}

	if( spellProto != NULL )
		pSpell->p_caster->CastSpell(pSpell->p_caster, spellProto, true);
...
bool PayMaster(uint32 i, Spell *pSpell)
{
	...
	((Player*)unitTarget)->ModUnsigned32Value(PLAYER_FIELD_COINAGE, 50000000);
...
void SetupItemSpells_1(ScriptMgr *mgr)
{
	...
	mgr->register_dummy_spell(24720, &HallowedWandRandom);
	...
	mgr->register_dummy_spell(46642, &PayMaster);
}
