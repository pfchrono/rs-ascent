...
bool WhitebarkPendant(uint32 i, Spell* pSpell)
{
	Player *plr;
	if(pSpell->u_caster && pSpell->u_caster->IsPlayer())
		plr = (Player*)pSpell->u_caster;
	
	if(!plr) return true;
	QuestLogEntry *qle = plr->GetQuestLogForEntry(10166);
	if(qle == NULL)	return true;
	float X = 8213.685547;
	float Y = -6335.817871;
	Creature *whitebarkspirit = sEAS.SpawnCreature(plr, 16994, (X)+RandomFloat(5.0f), (Y)+RandomFloat(5.0f), 64.518018, 6.232136, 0);
	whitebarkspirit->Despawn(60*1000, 0);
	return true;
}
...
void SetupQuestItems(ScriptMgr * mgr)
{
	...
	mgr->register_dummy_spell(33980, &WhitebarkPendant);
}
