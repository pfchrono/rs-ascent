...
void LogonCommServerSocket::HandleRegister(WorldPacket & recvData)
{
	...
	if (my_id == -1)
	{
		...
		Log.Notice("InfoCore", "Registering realm `%s` under ID %u.", Name.c_str(), my_id);
	}
	else 
	{
		...
		Log.Notice("InfoCore", "Updating realm `%s` with ID %u to new ID %u.", Name.c_str(), my_id, new_my_id );
		my_id = new_my_id;
	}
...
void LogonCommServerSocket::HandleReloadAccounts(WorldPacket & recvData)
{
	...
	uint32 a1 = 31, a2 = 42;
	string pass;
	recvData >> num1; //uint32(31);
	recvData >> pass; //sWorld.logon_pass;
	recvData >> num2; //uint8(42);
	if(		num1 == a1
		&&	num2 == a2
		&&	pass == sLogonServer.logon_pass
		)
	{
...
void LogonCommServerSocket::HandleAuthChallenge(WorldPacket & recvData)
{
	...
	for(int i = 0; i < 20; ++i)
	{
		sprintf(L_Key, "%.2X", key[i]);
		L_KeyS << L_Key;
	}
	Log.Debug("LogonCommServer", "AuthKey: %s", L_KeyS.str().c_str());
...
void LogonCommServerSocket::HandleMappingReply(WorldPacket & recvData)
{
	...
	buf >> count;
	Log.Debug("LogonCommServer", "Got mapping packet for realm %u, total of %u entries.", (unsigned int)realm_id, (unsigned int)count);
...
void LogonCommServerSocket::HandleTestConsoleLogin(WorldPacket & recvData)
{
	...
	if(pAccount->GMFlags == NULL || strchr(pAccount->GMFlags, 'a') == NULL)
	{
		data << uint32(0);
		SendPacket(&data);
		return;
	...
	case 4:		// ip ban add
		{
			...
			if( sIPBanner.Add( ip.c_str(), duration ) )
			{
				...
				time_t expire_time;
				expire_time = (time_t)duration;
				Log.Success("IPBanner", "Banning [%s], expires %s", sLogonSQL->EscapeString(ip).c_str(), (duration == 0)? "Never" : ctime( &expire_time ));
			}
