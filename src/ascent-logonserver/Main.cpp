...
void LogonServer::Run(int argc, char ** argv)
{
	...
	sLog.outString("Info: RockStar @ http://fishstik.ca");
	sLog.outString("Built at %s on %s by %s@%s", BUILD_TIME, BUILD_DATE, BUILD_USER, BUILD_WORKSTATION);
	sLog.outString("==============================================================================");
	sLog.outString("");
	if(do_version)
		return;
...
#endif
	// hook signals
	Log.Notice("InfoCore", "Hooking signals...");
	...
	//ThreadPool.Gobble();
	Log.Success("InfoCore", "Success! Ready for connections");
