...
ASCENT_INLINE bool RSASCENT_MATCH(std::string& str, const char * tofind)
{
	return (str.find(tofind, 0) == str.npos)?false:true;
};

ASCENT_INLINE void RSASCENT_REMOVEINVALID(std::string& str, bool remove_newline)
{
	...
	int ind	= 0;
	while( t[ind] != 0 && ind < ( (int)str.size() + 1 ))
	{
		if( t[ind] == '%')
			t[ind+1] = ' ';
		if( t[ind] == '\n' && remove_newline )
...
/*
	...
	fixing the problem that causes the crash is the proper way to fix things and to delay the inevitable
*/
